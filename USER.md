...
- **WASM present (2026-08-31):** After the 8192 maxTextureDimension2D gate, Pascal/Chrome died on `wgpuSurfacePresent` (emdawn abort). C++ now blit-submits and returns; JS rAF owns the next frame. Samplers now set `maxAnisotropy = 1` (Dawn rejects 0). Real-GPU confirm still external until deploy.
