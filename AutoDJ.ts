...
export class Alucinate {
  ...
  private getCurrentState: () => { currentImage: ImageRecord | null, currentShader: ShaderRecord | null };
  ...
  public async initialize(imageManifestUrl: string, shaderDefs: any[], imageSuggestionsUrl: string) {
    if (this.status === 'ready' || this.status === 'loading-models') return;

    this.setStatus('loading-models', 'Starting AI model initialization...');
    
    try {
        // 1. Load Manifests and Suggestions
        this.setStatus('loading-models', 'Fetching manifests and suggestions...');
        const [imgManifestResponse, suggestionsResponse] = await Promise.all([
            fetch(imageManifestUrl),
            fetch(imageSuggestionsUrl)
        ]);

        const manifestData = await imgManifestResponse.json();
        this.imageManifest = manifestData.images;
  ...
  public start() {
    ...
    this.isRunning = true;
    this.runCycle(); 
    this.loopInterval = window.setInterval(() => this.runCycle(), 25000); // Increased interval for more complex cycle
  }
  ...
  private async runCycle() {
    ...
    try {
        ...
        // Wait a moment before deciding the next image
...
Your selection (ID only):`;
    ...
    try {
        ...
        const match = choice.match(new RegExp('"([^"]+)"'));
...
Describe the next scene in a single, descriptive sentence.`;
  ...
  private findBestMatchingImage(theme: string, currentUrl: string): ImageRecord {
      ...
      for (const image of candidates) {
          ...
          // Bonus for having more tags that match
          if (score > 0) {
      ...
      // Fallback to a random image if no match is found
