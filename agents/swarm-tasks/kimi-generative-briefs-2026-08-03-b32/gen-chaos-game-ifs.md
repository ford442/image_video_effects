# Swarm Brief — gen-chaos-game-ifs (batch b32, role: algorithmist)

- **ID**: gen-chaos-game-ifs
- **Name**: Chaos Game IFS
- **Category**: generative
- **Description**: Iterated function system chaos game generating fractal attractors. Audio drives iteration depth and attractor scale. Rotating attractor points.
- **Current lines**: 134  |  **Target**: 194–224 (+60 to +90)

## Existing updatedParams (preset contract — keep names/defaults EXACTLY, indices 0-3)
  - index 0: "Iterations" default=0.5 min=0 max=1 step=0.01
  - index 1: "Glow Size" default=0.5 min=0 max=1 step=0.01
  - index 2: "Ring Count" default=0.3 min=0 max=1 step=0.01
...
1. `gen-chaos-game-ifs.wgsl` — complete, self-contained upgraded shader (canonical header).
2. `gen-chaos-game-ifs.json` — complete upgraded metadata.
3. `gen-chaos-game-ifs.md` — changelog: what changed & why, loop budget/perf estimate, rating prediction.
