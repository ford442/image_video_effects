# Swarm Brief — gen-aperiodic-monotile (batch b32, role: algorithmist)

- **ID**: gen-aperiodic-monotile
- **Name**: Aperiodic Monotile Hat
- **Category**: generative
- **Description**: Aperiodic hat tiling (monotile) with animated edge glow and audio-reactive color cycling. Rotates and scales with parameters.
- **Current lines**: 138  |  **Target**: 198–228 (+60 to +90)

## Existing updatedParams (preset contract — keep names/defaults EXACTLY, indices 0-3)
  - index 0: "Tile Scale" default=0.5 min=0 max=1 step=0.01
  - index 1: "Rotation Speed" default=0.5 min=0 max=1 step=0.01
  - index 2: "Edge Width" default=0.3 min=0 max=1 step=0.01
  - index 3: "Saturation" default=0.5 min=0 max=1 step=0.01
...
1. `gen-aperiodic-monotile.wgsl` — complete, self-contained upgraded shader (canonical header).
2. `gen-aperiodic-monotile.json` — complete upgraded metadata.
3. `gen-aperiodic-monotile.md` — changelog: what changed & why, loop budget/perf estimate, rating prediction.
