# Shader Upgrade Task: `data-scanner`

## Metadata
- **Shader ID**: data-scanner
- **Agent Role**: Algorithmist
- **Current Size**: 3405 bytes
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let param1 = u.zoom_params.x; // Intensity
    let param2 = u.zoom_params.y; // Audio Reactivity
    let param3 = u.zoom_params.z; // Scale
    let param4 = u.zoom_params.w; // Detail
    ...
    let edgeRaw = sobel(uv, texel);
    let edge = edgeRaw * (1.0 + (param4 - 0.5) * 2.0);
    let lum = get_luminance(color);

    if (in_scan > 0.0) {
        // Analysis Mode
        let gridFreq = 40.0 * (1.0 + (param3 - 0.5) * 1.0);
        let grid_uv = fract(uv * gridFreq);
        ...
        // Highlight the scan bar edges with audio reactivity
        let audioPulse = 1.0 + bass * param2 * 4.0;
        let borderGlow = 0.005 * (1.0 + bass * param2 * 0.5);
        let border_line = smoothstep(scan_width - borderGlow, scan_width, dist) * (1.0 - smoothstep(scan_width, scan_width + borderGlow, dist));
        analyzed += vec3<f32>(1.0, 1.0, 1.0) * border_line * 4.0 * audioPulse;

        let intensity = 0.9 * (0.5 + param1);
        color = mix(color, analyzed, in_scan * intensity);
    } else {
        // Dim outside
    ...
    // Meaningful alpha: higher inside scan, boosted by luminance and edge intensity
