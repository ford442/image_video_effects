# KIMI SWARM TASK — REPAIR — gen-fireworks-comet-trail
...
## CREATIVE BRIEF
This shader renders comet-trail fireworks. It has clean bindings and already reads `plasmaBuffer`, but it lacks a modern Standard Hybrid Header. Upgrade: add a full header, enhance bass-driven spawn rate and head brightness, let treble increase the peel-off sparkle count, and ensure the final write uses premultiplied alpha. Preserve the comet color cycling and hex-bokeh glow.

## OUTPUT CONTRACT
1. After closing ```: empty.
2. Exact 13-binding header below.
3. Standard Hybrid Header with Category `generative`, Features `audio-reactive, mouse-driven, fireworks, temporal`.
4. Use premultiplied alpha writeback.
5. Add IGN dither after ACES.
...
## ROLE TOOLKIT — Visualist + Optimizer
- Add a full Standard Hybrid Header at the top.
- Keep existing audio reads but make them more impactful: bass should also scale head brightness and spawn timing.
- Add `ign` dither after ACES.
- Convert final `textureStore(writeTexture, ...)` to premultiplied alpha when alpha < 1.0.
- Keep all existing math; this is a header + polish pass.
...
Target: ≤ 180 lines. Preserve comet character; focus on header and audio polish.
