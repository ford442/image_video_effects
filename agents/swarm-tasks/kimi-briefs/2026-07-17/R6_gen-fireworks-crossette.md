# KIMI SWARM TASK — REPAIR — gen-fireworks-crossette
...
## CREATIVE BRIEF
This shader renders four-arm crossette split fireworks. It has clean bindings and already reads `plasmaBuffer`, but lacks a modern Standard Hybrid Header. Upgrade: add a full header, let bass accelerate the arm split timing, let treble brighten the sub-spark intensity, and add IGN dither + premultiplied alpha. Preserve the four-arm color pattern and gravity-driven sparks.
...
4. Premultiplied alpha writeback.
5. IGN dither after ACES.
...
## ROLE TOOLKIT
- Add Standard Hybrid Header.
- Make `splitDly` additionally modulated by bass (faster split on beats).
- Scale arm sub-spark intensity with treble.
- Add IGN dither after ACES.
- Premultiplied alpha on final write.
...
Target: ≤ 190 lines. Preserve crossette character.
