# KIMI SWARM TASK — REPAIR — gen-fireworks-strobe-shell
...
## CREATIVE BRIEF
This shader renders rhythmic strobe-blink fireworks. It has clean bindings and already reads `plasmaBuffer`, but lacks a modern Standard Hybrid Header. Upgrade: add a full header, enhance the strobe pulse to be more responsive to bass + mids, add treble-driven sparkle count, and ensure premultiplied alpha + IGN dither. Preserve the white/core flash and radial spark pattern.

## OUTPUT CONTRACT
1. After closing ```: empty.
2. Exact 13-binding header below.
3. Standard Hybrid Header with Category `generative`, Features `audio-reactive, mouse-driven, fireworks, temporal, strobe`.
...
- Make `strobePulse` frequency also responsive to mids (not just bass).
...
Target: ≤ 190 lines. Preserve strobe character.
