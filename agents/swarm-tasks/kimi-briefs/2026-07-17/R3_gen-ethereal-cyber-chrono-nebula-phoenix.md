# KIMI SWARM TASK — REPAIR — gen-ethereal-cyber-chrono-nebula-phoenix
...
## CREATIVE BRIEF
This shader renders an ethereal cyber-phoenix over a domain-warped nebula. It has a critical silent bug: it reads `u.config.y` as audio, which is actually `MouseClickCount`, not music. Replace it with proper `plasmaBuffer[0]` reads. Add bass-driven nebula turbulence, mids-driven chrono-glow, and treble-driven edge sparkle. Refresh the header to the modern Standard Hybrid Header format. Preserve the deep purple/neon palette and the SDF phoenix silhouette.

## DIFFERENTIATE FROM
- `gen-chrono-voronoi-mycelium`: fungal growth — yours is a celestial bird.
- `gen-auroral-ferrofluid-monolith`: magnetic fluid glyphs — yours is nebula + phoenix.

## OUTPUT CONTRACT (non-negotiable)
1. After the closing ``` of the WGSL block: completely empty.
2. Use the exact 13-binding header below.
3. Replace every `u.config.y` used as audio with `plasmaBuffer[0].x/y/z`.
4. Alpha must carry semantic meaning (density + glow).
5. Include a modern Standard Hybrid Header.
...
## ROLE TOOLKIT — Algorithmist + Interactivist
- Replace `let audio = u.config.y;` with `let bass = plasmaBuffer[0].x; let mids = plasmaBuffer[0].y; let treble = plasmaBuffer[0].z;`.
- Use `bass` where `audio` was used for nebula turbulence and phoenix color boost.
- Use `mids` to drive chrono-glow intensity.
- Use `treble` for edge sparkle / shell brightness.
- Add `acesToneMap` + `ign` dither before the final write.
- Refresh the header to the Standard Hybrid Header format.
...
Target: ≤ 180 lines. Fix the audio bug; do not rewrite the whole shader.
