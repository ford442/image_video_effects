# KIMI SWARM TASK — REPAIR — gen-quasicrystal-iridescence
...
## CREATIVE BRIEF
This shader renders Penrose-tiling-inspired quasicrystal patterns with thin-film iridescence. It has clean bindings and a good header, but zero audio reactivity and uses a simple Reinhard tone map. Upgrade it so bass rotates the projection angle and thickens the film, mids shift the color cycle, and treble adds shimmer to the metallic edges. Preserve the soap-bubble spectral colors and the aperiodic 5-fold symmetry. Alpha should encode pattern density / film thickness.

## DIFFERENTIATE FROM
- `gen-quasicrystal`: the base quasicrystal pattern — your version must keep the iridescence layer as the dominant visual feature.
- `gen-auroral-ferrofluid-monolith`: magnetic glyph formation — yours is crystalline/spectral, not fluid.

## OUTPUT CONTRACT (non-negotiable)
1. After the closing ``` of the WGSL block: completely empty.
2. Use the exact 13-binding header below.
3. Alpha must carry semantic meaning (pattern density or film thickness).
4. Use at least two tactics (ACES + IGN dither + audio reactivity).
5. Keep the Standard Hybrid Header; refresh Created date and Features.
...
- Add `bass`, `mids`, `treble` from `plasmaBuffer[0]`.
- Let bass modulate `projAngle` rotation speed and film thickness range.
- Let mids shift the metallic color cycle.
- Let treble add edge shimmer / sparkle.
- Replace the simple `outColor / (1.0 + outColor * 0.2)` tone map with `acesToneMap` + `ign` dither.
- Keep mouse interaction as-is or make it slightly more reactive to bass.
...
Target: ≤ 200 lines. Preserve the iridescent quasicrystal character.
