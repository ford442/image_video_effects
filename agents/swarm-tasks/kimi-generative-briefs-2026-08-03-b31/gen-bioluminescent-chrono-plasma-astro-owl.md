# Swarm Brief — gen-bioluminescent-chrono-plasma-astro-owl (batch b31, role: interactivist)

- **ID**: gen-bioluminescent-chrono-plasma-astro-owl
- **Name**: Bioluminescent Chrono-Plasma Astro-Owl
- **Category**: generative
- **Description**: A hyper-dimensional, cyber-organic celestial owl forged from liquid chrono-plasma and shattered quantum-glass.
- **Current lines**: 260  |  **Target**: 320–350 (+60 to +90)

## Existing updatedParams (preset contract — keep names/defaults EXACTLY, indices 0-3)
  - index 0: "Wing Plasma Distortion" default=1.0 min=0.1 max=5.0 step=0.1
  - index 1: "Core Gravity Intensity" default=0.5 min=0.0 max=2.0 step=0.05
  - index 2: "Nebula Particle Density" default=0.8 min=0.0 max=3.0 step=0.1
  - index 3: "Temporal Echo Fade" default=0.2 min=0.0 max=1.0 step=0.01
...
1. `gen-bioluminescent-chrono-plasma-astro-owl.wgsl` — complete, self-contained upgraded shader (canonical header).
2. `gen-bioluminescent-chrono-plasma-astro-owl.json` — complete upgraded metadata.
3. `gen-bioluminescent-chrono-plasma-astro-owl.md` — changelog: what changed & why, loop budget/perf estimate, rating prediction.
