# Swarm Brief — gen-radiant-cyber-plasma-astro-griffin (batch b31, role: visualist)

- **ID**: gen-radiant-cyber-plasma-astro-griffin
- **Name**: Radiant Cyber-Plasma Astro-Griffin
- **Category**: generative
- **Description**: A hyper-majestic, biomechanical celestial griffin woven from shattered chrono-prisms and glowing liquid-plasma.
- **Current lines**: 228  |  **Target**: 288–318 (+60 to +90)

## Existing updatedParams (preset contract — keep names/defaults EXACTLY, indices 0-3)
  - index 0: "Warp Intensity" default=0.5 min=0.0 max=1.0 step=0.01
  - index 1: "Wing Span" default=0.7 min=0.1 max=1.5 step=0.01
  - index 2: "Fractal Depth" default=0.4 min=0.0 max=1.0 step=0.05
  - index 3: "Plasma Glow" default=0.8 min=0.0 max=2.0 step=0.05
...
1. `gen-radiant-cyber-plasma-astro-griffin.wgsl` — complete, self-contained upgraded shader (canonical header).
2. `gen-radiant-cyber-plasma-astro-griffin.json` — complete upgraded metadata.
3. `gen-radiant-cyber-plasma-astro-griffin.md` — changelog: what changed & why, loop budget/perf estimate, rating prediction.
