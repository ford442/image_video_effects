# Swarm Brief — gen-prismatic-cyber-aether-void-kitsune (batch b31, role: algorithmist)

- **ID**: gen-prismatic-cyber-aether-void-kitsune
- **Name**: Prismatic Cyber-Aether Void-Kitsune
- **Category**: generative
- **Description**: A hyper-majestic, cybernetic celestial kitsune forged from shattered prismatic glass and liquid quantum-aether, bounding violently through a multi-dimensional volumetric particle-storm while its nine radiant tails carve temporal auroral rifts into the void.
- **Current lines**: 309  |  **Target**: 369–399 (+60 to +90)

## Existing updatedParams (preset contract — keep names/defaults EXACTLY, indices 0-3)
  - index 0: "Tail Dispersion" default=1.0 min=0.1 max=3.0 step=0.05
  - index 1: "Current Warp" default=0.6 min=0.0 max=2.0 step=0.05
  - index 2: "Storm Density" default=0.5 min=0.0 max=1.0 step=0.01
  - index 3: "Rune Glow" default=1.2 min=0.0 max=3.0 step=0.1
...
1. `gen-prismatic-cyber-aether-void-kitsune.wgsl` — complete, self-contained upgraded shader (canonical header).
2. `gen-prismatic-cyber-aether-void-kitsune.json` — complete upgraded metadata.
3. `gen-prismatic-cyber-aether-void-kitsune.md` — changelog: what changed & why, loop budget/perf estimate, rating prediction.
