# Swarm Brief — gen-luminescent-quantum-flora-symphony (batch b31, role: optimizer)

- **ID**: gen-luminescent-quantum-flora-symphony
- **Name**: Luminescent Quantum-Flora Symphony
- **Category**: generative
- **Description**: 
- **Current lines**: 103  |  **Target**: 163–193 (+60 to +90)

## Existing updatedParams (preset contract — keep names/defaults EXACTLY, indices 0-3)
  - index 0: "Petal Complexity" default=0.5 min=0.1 max=5.0 step=0.1
  - index 1: "Gravity Twist" default=0.5 min=0.0 max=2.0 step=0.05
  - index 2: "Spore Density" default=0.5 min=0.0 max=2.0 step=0.05
  - index 3: "Core Intensity" default=0.5 min=0.1 max=4.0 step=0.1
...
1. `gen-luminescent-quantum-flora-symphony.wgsl` — complete, self-contained upgraded shader (canonical header).
2. `gen-luminescent-quantum-flora-symphony.json` — complete upgraded metadata.
3. `gen-luminescent-quantum-flora-symphony.md` — changelog: what changed & why, loop budget/perf estimate, rating prediction.
