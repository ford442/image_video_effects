# Swarm Brief — gen-sentient-cyber-chrono-void-serpent (batch b31, role: algorithmist)

- **ID**: gen-sentient-cyber-chrono-void-serpent
- **Name**: Sentient Cyber-Chrono Void-Serpent
- **Category**: generative
- **Description**: A hyper-dimensional, endlessly twisting biomechanical serpent forged from shattered chrono-glass and liquid auroral plasma, slithering aggressively through a violently decaying temporal cosmic-rift. This shader blends razor-sharp synthetic scales and recursive geometric fracturing with fluid, audio-reactive bioluminescent shockwaves.
- **Current lines**: 233  |  **Target**: 293–323 (+60 to +90)

## Existing updatedParams (preset contract — keep names/defaults EXACTLY, indices 0-3)
  - index 0: "Time Scale" default=0.5 min=0.0 max=5.0 step=0.1
  - index 1: "Plasma Intensity" default=0.5 min=0.0 max=3.0 step=0.1
  - index 2: "Void Density" default=0.5 min=0.0 max=1.0 step=0.05
  - index 3: "Fracture Depth" default=0.5 min=0.0 max=10.0 step=0.5
...
1. `gen-sentient-cyber-chrono-void-serpent.wgsl` — complete, self-contained upgraded shader (canonical header).
2. `gen-sentient-cyber-chrono-void-serpent.json` — complete upgraded metadata.
3. `gen-sentient-cyber-chrono-void-serpent.md` — changelog: what changed & why, loop budget/perf estimate, rating prediction.
