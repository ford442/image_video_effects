# Swarm Brief — gen-radiant-cyber-bismuth-nebula-colossus (batch b31, role: optimizer)

- **ID**: gen-radiant-cyber-bismuth-nebula-colossus
- **Name**: Radiant Cyber-Bismuth Nebula-Colossus
- **Category**: generative
- **Description**: A colossal, hyper-dimensional biomechanical titan woven from shattered cyber-bismuth lattices and liquid auroral plasma, drifting endlessly through a violently decaying quantum-particle storm.
- **Current lines**: 109  |  **Target**: 169–199 (+60 to +90)

## Existing updatedParams (preset contract — keep names/defaults EXACTLY, indices 0-3)
  - index 0: "Fractal Fold Scale" default=1.0 min=0.1 max=3.0 step=0.01
  - index 1: "Nebula Density" default=0.5 min=0.0 max=1.0 step=0.01
  - index 2: "Audio Reactivity" default=1.0 min=0.0 max=5.0 step=0.1
  - index 3: "Iridiscent Shift" default=0.5 min=0.0 max=1.0 step=0.01
...
1. `gen-radiant-cyber-bismuth-nebula-colossus.wgsl` — complete, self-contained upgraded shader (canonical header).
2. `gen-radiant-cyber-bismuth-nebula-colossus.json` — complete upgraded metadata.
3. `gen-radiant-cyber-bismuth-nebula-colossus.md` — changelog: what changed & why, loop budget/perf estimate, rating prediction.
