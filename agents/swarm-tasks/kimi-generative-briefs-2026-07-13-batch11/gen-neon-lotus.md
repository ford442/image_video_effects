# Swarm Brief: gen-neon-lotus

**Role:** Interactivist
**Name:** Neon Lotus
**Category:** generative
**Description:** Blooming neon lotus with layered petals, mouse-shifted center, audio-driven bloom/glow, and prismatic petal coloring.
**Current lines:** 161
**Target lines:** 220–300 (expand by +60 to +140)

## Role Instructions

You are the Interactivist. Focus on input reactivity, feedback loops, and emergent behavior:
- Add click-triggered petal bloom/shockwave from the lotus center.
- Add gravity-well mouse attraction that warps petals toward the cursor.
- Map bass to bloom expansion, mids to hue shift speed, treble to edge sparkle.
- Add temporal feedback so petals leave brief neon trails as they rotate.
- Add luma-keyed or depth-keyed particle spawn from the video input around petal edges.
- Add audio envelope followers (attack/release) so reactivity feels smooth, not jittery.
- Preserve the original "soul" — a blooming neon lotus.
...
```json
[
  {"index": 0, "name": "Petal Count", "default": 0.4, "min": 0, "max": 1, "step": 0.01},
  {"index": 1, "name": "Bloom", "default": 0.5, "min": 0, "max": 1, "step": 0.01},
  {"index": 2, "name": "Speed", "default": 0.3, "min": 0, "max": 1, "step": 0.01},
  {"index": 3, "name": "Glow Scale", "default": 0.4, "min": 0, "max": 1, "step": 0.01}
]
```
