# Swarm Brief: gen-prismatic-ion-cascade

**Role:** Optimizer
**Name:** Prismatic Ion Cascade
**Category:** generative
**Description:** Mouse-anchored cascade of prismatic ion streams with curl-warped angular bands, chromatic spectral split, FBM shimmer, and audio-reactive flicker.
**Current lines:** 147
**Target lines:** 210–290 (expand by +70 to +150)

## Role Instructions

You are the Optimizer. Focus on performance, elegance, and pipeline integration:
- Refactor band calculation into helpers with named constants and reduce redundant sin() calls.
- Add LOD quality scaling: fewer FBM octaves / coarser bands in low-alpha or distant regions.
- Add early-exit for pixels outside the cascade influence radius.
- Add hex-bokeh or sparse sample glow approximation to replace full-scene FBM shimmer.
- Add smart depth compositing: sample `readDepthTexture` and blend ion cascade with scene depth.
- Expose bloom threshold metadata and tag shader as HDR-ready.
- Preserve the original "soul" — radiating prismatic ion streams from the cursor.
...
```json
[
  {"index": 0, "name": "Stream Density", "default": 0.5, "min": 0, "max": 1, "step": 0.01},
  {"index": 1, "name": "Cascade Speed", "default": 0.5, "min": 0, "max": 1, "step": 0.01},
  {"index": 2, "name": "Spectral Spread", "default": 0.4, "min": 0, "max": 1, "step": 0.01},
  {"index": 3, "name": "Ion Thickness", "default": 0.35, "min": 0, "max": 1, "step": 0.01}
]
```
