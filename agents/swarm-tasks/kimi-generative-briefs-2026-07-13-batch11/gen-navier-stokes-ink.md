# Swarm Brief: gen-navier-stokes-ink

**Role:** Algorithmist
**Name:** Navier-Stokes Ink
**Category:** generative
**Description:** 2D Navier-Stokes fluid simulation with semi-Lagrangian advection, pressure projection stub, ink injection, and audio-reactive vorticity.
**Current lines:** 133
**Target lines:** 200–280 (expand by +70 to +150)

## Role Instructions

You are the Algorithmist. Focus on mathematical/algorithmic depth:
- Add a second Jacobi pressure-projection iteration to improve divergence-free velocity.
- Replace simple value noise with curl-noise velocity perturbation or domain-warped FBM advection.
- Add density diffusion with anisotropic kernels aligned to flow direction.
- Add a procedural dye SDF layer (e.g., ink drop collisions, filament stretching).
- Use branchless select()/mix() instead of per-pixel if blocks.
- Preserve the original "soul" — turbulent ink flow reacting to audio and mouse.
...
- Write to `writeTexture`, `writeDepthTexture`, `dataTextureA`, and `dataTextureB` every frame.
...
```json
[
  {"index": 0, "name": "Injection Rate", "default": 0.4, "min": 0, "max": 1, "step": 0.01},
  {"index": 1, "name": "Viscosity", "default": 0.35, "min": 0, "max": 1, "step": 0.01},
  {"index": 2, "name": "Dispersion", "default": 0.3, "min": 0, "max": 1, "step": 0.01},
  {"index": 3, "name": "Vorticity Scale", "default": 0.5, "min": 0, "max": 1, "step": 0.01}
]
```
