# Swarm Brief: gen-bioluminescent-reaction-diffusion

**Role:** Algorithmist
**Name:** Bioluminescent Reaction-Diffusion
**Category:** generative
**Description:** Gray-Scott reaction-diffusion simulation with bioluminescent color mapping, mouse seeding, and audio-driven feed/kill rates.
**Current lines:** 131
...
- Replace the simple 3x3 laplacian with a higher-quality 5x5 Laplacian kernel or multiple neighbor-weight sets.
- Add multi-scale reaction-diffusion: simulate two competing species or layered feed/kill regimes.
- Add domain-warped FBM or curl-noise advection to the chemical field.
- Add a procedural "plankton/bacteria" SDF layer or strange-attractor orbit trap for complexity.
- Use branchless select()/mix() instead of per-pixel if blocks.
- Preserve the original "soul" — a living, glowing reaction-diffusion pattern.
...
```json
[
  {"index": 0, "name": "Intensity", "default": 0.5, "min": 0, "max": 1, "step": 0.01},
  {"index": 1, "name": "Speed", "default": 0.5, "min": 0, "max": 1, "step": 0.01},
  {"index": 2, "name": "Scale", "default": 0.5, "min": 0, "max": 1, "step": 0.01},
  {"index": 3, "name": "Mouse Influence", "default": 0.5, "min": 0, "max": 1, "step": 0.01}
]
```
