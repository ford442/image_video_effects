# Swarm Brief: gen-crystal-lattice-growth

**Role:** Optimizer
**Name:** Crystal Lattice Growth
**Category:** generative
**Description:** Golden-ratio dendritic crystal growth with radial symmetry, prismatic hue, audio reactivity, and depth output.
**Current lines:** 150
...
- Refactor the recursive branch SDF into a cleaner loop with named constants and branchless selection.
- Add LOD quality scaling: fewer branch iterations / lower symmetry when far from center or at high zoom.
- Add early-exit for pixels far from any crystal energy.
- Add hex-bokeh or sample-sparse glow post-process approximation to reduce per-pixel cost.
- Add smart depth pass-through that respects the crystal foreground.
- Expose bloom threshold metadata via comments and tag the shader as HDR-ready.
- Preserve the original "soul" — elegant crystalline dendrite growth.
...
```json
[
  {"index": 0, "name": "Symmetry", "default": 0.4, "min": 0, "max": 1, "step": 0.01},
  {"index": 1, "name": "Growth Rate", "default": 0.5, "min": 0, "max": 1, "step": 0.01},
  {"index": 2, "name": "Hue", "default": 0.6, "min": 0, "max": 1, "step": 0.01},
  {"index": 3, "name": "Thickness", "default": 0.4, "min": 0, "max": 1, "step": 0.01}
]
```
