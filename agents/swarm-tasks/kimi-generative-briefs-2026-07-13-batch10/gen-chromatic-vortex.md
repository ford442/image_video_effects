# Swarm Brief: gen-chromatic-vortex

**Role:** Visualist
**Name:** Chromatic Vortex
**Category:** distortion (treat as generative visual upgrade)
**Description:** Polar distortion and color-space warp with chromatic sector dispersion and temporal feedback.
**Current lines:** 133
**Target lines:** 190–260 (expand by +60 to +130)

## Role Instructions

You are the Visualist. Focus on color science, lighting, and emotional impact:
- Add ACES or Reinhard tone mapping with HDR headroom (allow values > 1.0 before tone map).
- Add atmospheric depth: volumetric fog, depth-scaled light rays, or Mie scattering along the vortex tunnel.
- Add Fresnel rim / iridescent thin-film effects to the swirling boundaries.
- Add dynamic color temperature shifts driven by bass/mids/treble.
- Add bloom/glow approximation using hex-bokeh or Kuwahara-like highlight blur.
- Use OkLab or YUV color warping for smoother hue rotations.
- Preserve the original "soul" — a hypnotic chromatic vortex.
...
No existing JSON definition. If you create one, use these parameter slots:
```json
[
  {"index": 0, "name": "Swirl Strength", "default": 0.5, "min": 0, "max": 1, "step": 0.01},
  {"index": 1, "name": "Radius Scale", "default": 0.5, "min": 0, "max": 1, "step": 0.01},
  {"index": 2, "name": "Polar Distort", "default": 0.5, "min": 0, "max": 1, "step": 0.01},
  {"index": 3, "name": "Color Warp", "default": 0.5, "min": 0, "max": 1, "step": 0.01}
]
```
