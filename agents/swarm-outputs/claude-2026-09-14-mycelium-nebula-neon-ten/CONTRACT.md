# Mycelium / nebula / neon generative ten — agent contract (2026-09-14)
...
Two **native** ideas each — grounded in the shader's own subject (fungal hyphal growth & mushroom morphology, Navier–Stokes ink transport, nebular emission & astrolabe mechanics, neon gas-discharge physics, lotus botany, neural spiking dynamics, biomechanical hive architecture…), not generic noise/bloom/IQ-palette overlays. Keep identity and existing core functions. No new springs. No replacing the motif.
...
## Card
Write an Idea Card section for each of your shaders to `agents/swarm-outputs/claude-2026-09-14-mycelium-nebula-neon-ten/card-<id>.md`:
```
SHADER: <id>
IDENTITY: ...
KEEP VERBATIM: ...
...
Some shaders here (e.g. gen-navier-stokes-ink, gen-mycelium-network, gen-nebula-light-trail-swarm) carry simulation state through dataTextureA -> dataTextureC. If the current A packing is sim state (velocity/dye/growth), do not silently destroy the sim: either keep the sim state recoverable from the display RGBA (e.g. dye in rgb, density in alpha) or document the exact packing on the `A packing:` header line and card. Where bilinear sampling of C was essential (semi-Lagrangian advection), replace `textureSampleLevel(dataTextureC, ...)` with a manual bilinear built from 4 clamped `textureLoad` calls — still exact loads, no sampler.
