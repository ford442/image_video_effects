...
def run(mode, steps=1500, seed=3, p=(0.5,0.5,0.5,0.5)):
    ...
    for t in range(steps):
        ...
        if mode=='nutrient': g=g+(lum-0.5)*0.9
        c=np.clip(center+gr*g+packet,0,1)
        S=ss(thr*0.5,max(thr,0.01),c)
        if t%300==299: hist.append((t,S.mean(),S[:,:N//3].mean(),S[:,-N//3:].mean(),S.std()))
    return hist
for m in sys.argv[1:]:
    print(m); [print('  t=%d mean %.3f  darkThird %.3f  brightThird %.3f  std %.3f'%h) for h in run(m)]
