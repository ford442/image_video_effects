# Neural / plasma / photonic generative ten — agent contract (2026-09-14)
...
Two **native** ideas each — grounded in the shader's own subject (neural fractal branching & dendritic growth, synaptic vesicle release / neurotransmitter diffusion, neuronal cosmology, MHD plasma & lotus phyllotaxis, liquid-metal gold optics (thin-film, Fresnel on conductors), neutron-star magnetospheres (dipole field lines, pulsar beams, curvature radiation), Newton-method basins & convergence rates, volcanic glass conchoidal fracture / acoustic echo & reverberation, opal play-of-colour (Bragg diffraction from silica spheres), PCB circuit traces & signal propagation…), not generic noise/bloom/IQ-palette overlays. Keep identity and existing core functions. No new springs. No replacing the motif.
...
## Card
Write an Idea Card section for each of your shaders to `agents/swarm-outputs/claude-2026-09-14-neural-plasma-photonic-ten/card-<id>.md`:
```
SHADER: <id>
IDENTITY: ...
KEEP VERBATIM: ...
...
- Survey at start: gen-neuro-cosmos and gen-neutron-star-magnetic-spindle have ZERO plasmaBuffer reads; gen-neural-fractal, gen-neuro-cosmos, gen-neutron-star-magnetic-spindle have NO dataTextureA write. gen-neuro-fluid-plasma-lotus and gen-opal-circuit read extraBuffer[0] (must relocate to guarded 133..138 or remove). gen-neuro-kinetic-bloom and gen-neuro-kinetic-liquid-gold-lotus already use 133..138 — verify guards and that they aren't fake audio. Several read u.config.y more than once — check for click-count-as-audio misuse.
- plasmaBuffer reads beyond index 0 (plasmaBuffer[1..8] as "FFT bins") are fake audio — replace with plasmaBuffer[0].xyz.
- If any shader keeps sim state in A, document the exact packing on the `A packing:` line; where bilinear sampling of C was essential, build a manual bilinear from 4 clamped `textureLoad` calls.
