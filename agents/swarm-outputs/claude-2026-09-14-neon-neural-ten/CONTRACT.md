# Neon / neural generative ten — agent contract (2026-09-14)
...
Two **native** ideas each — grounded in the shader's own subject (snow crystal habit & fall dynamics, polyhedral star geometry, tropical reef/ocean optics, neural spiking & bioluminescent chemistry, Koch fractal self-similarity, Langton ant turmite rules, Lenia continuous CA kernels, crystal lattice optics, nebular emission…), not generic noise/bloom/IQ-palette overlays. Keep identity and existing core functions. No new springs. No replacing the motif.
...
## Card
Write an Idea Card section for each of your shaders to `agents/swarm-outputs/claude-2026-09-14-neon-neural-ten/card-<id>.md`:
```
SHADER: <id>
IDENTITY: ...
KEEP VERBATIM: ...
...
## Simulation-state note (this batch)
gen-langton-ant and gen-lenia-2 (and possibly gen-neural-dust / snowfall trails) carry simulation state through dataTextureA -> dataTextureC. If A currently holds sim state (ant grid/direction, Lenia species mass), do not silently destroy the sim: keep it recoverable from A or document the exact packing on the `A packing:` header line and card; writeTexture then holds the ACES display. Where bilinear sampling of C was essential, build a manual bilinear from 4 clamped `textureLoad` calls.
...
- IDs: `gen-kimi-crystal` / `gen-kimi-nebula` live at `public/shaders/gen_kimi_crystal.wgsl`, `gen_kimi_nebula.wgsl` and `shader_definitions/generative/gen_kimi_*.json` (underscores in filenames). Cards use the file basename.
