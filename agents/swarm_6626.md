...
| **4D bass_env migration** | To Do | 4D | 8 | Replace raw `plasmaBuffer[0].x` with `bass_env` on Batch 1 shaders |
| **3E polish E1–E3** | In Progress | 3E | 2/3 done | E1 `gen-translucent-nebula` ✅: distance layer-LOD (8→5), branchless `hueToRGB`, `bass_env`-driven CA, feature sync. E2 `gen-prismatic-crystal-growth` ✅: ray-step LOD (`mix(0.7,1.0,...)`), caustics early-exit gate (`causticStrength*thickness>0.04`), branchless `hueToRGB`, `bass_env`-driven CA, `distance-lod` added. Notes in `claude-notes/`. Next: E3 `electric-eel-storm` |
