...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  let trail = exp(-(local.x - flakePos.x) * (local.x - flakePos.x) * 130.0) * saturate(0.45 - (local.y - flakePos.y)) * streak;
  ...
  let presence = saturate(flake * 0.95 + trail * 0.6);
  let alpha = saturate(0.06 + presence * 0.94);
  let depth = saturate(0.92 - flake * 0.65 - trail * 0.25);
