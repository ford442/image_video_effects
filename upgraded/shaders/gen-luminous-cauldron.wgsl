...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  let bubbleRnd = hash3(bubbleGrid).x;
  ...
  let sparks = step(0.997 - treble * 0.03, hash3(floor((uv + vec2<f32>(time * 0.04, -time * 0.03)) * 260.0)).x);
  ...
  let presence = saturate(bowl * 0.8 + froth * 0.7 + sparks);
  let alpha = saturate(0.08 + presence * 0.92);
  let depth = saturate(0.9 - bowl * 0.6 - froth * 0.25);
