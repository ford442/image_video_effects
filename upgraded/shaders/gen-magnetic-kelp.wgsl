...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  // Inline hash replaced with hash3 chunk
  let seed = hash3(vec2<f32>(laneId, 13.7)).x;
  ...
  // Inline hash replaced with hash3 chunk
  let spores = step(0.998 - treble * 0.03, hash3(floor((uv + vec2<f32>(time * 0.03, -time * 0.05)) * 280.0)).x);
  ...
  // saturate() replaces clamp(x, 0.0, 1.0)
  let presence = saturate(strand * 0.75 + frond * 0.8 + spores);
  let alpha = saturate(0.1 + presence * 0.9);
  let depth = saturate(0.95 - strand * 0.45 - frond * 0.35);
