...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  let spores = step(0.994 - treble * 0.03, hash3(floor((uv + time * 0.03) * 300.0)).x);
  ...
  let presence = saturate(colony * 0.9 + poisonCloud * 0.45 + spores * 0.8);
  let alpha = saturate(0.15 + presence * 0.85);
  let depth = saturate(0.88 - nucleus * 0.55 + poisonCloud * 0.2);
