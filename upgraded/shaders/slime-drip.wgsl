...
fn noise(p: vec2<f32>) -> f32 {
    ...
    let f = fract(p);
    let u = f * f * (3.0 - 2.0 * f);
    return mix(mix(dot(vec2<f32>(hash3(i + vec2<f32>(0.0, 0.0)).x, hash3(i + vec2<f32>(0.5, 0.0)).x), f - vec2<f32>(0.0, 0.0)),
                   dot(vec2<f32>(hash3(i + vec2<f32>(1.0, 0.0)).x, hash3(i + vec2<f32>(1.5, 0.0)).x), f - vec2<f32>(1.0, 0.0)), u.x),
               mix(dot(vec2<f32>(hash3(i + vec2<f32>(0.0, 1.0)).x, hash3(i + vec2<f32>(0.5, 1.0)).x), f - vec2<f32>(0.0, 1.0)),
                   dot(vec2<f32>(hash3(i + vec2<f32>(1.0, 1.0)).x, hash3(i + vec2<f32>(1.5, 1.0)).x), f - vec2<f32>(1.0, 1.0)), u.x), u.y);
}
...
fn mucusSSS(viewDir: vec3<f32>, lightDir: vec3<f32>, thickness: f32,
    ...
    // pow(x, 4.0) → x*x*x*x for integer exponents
    let fd2 = forwardDot * forwardDot;
    let forwardScatter = fd2 * fd2 * MUCUS_SCATTERING;
...
fn calculateSlimeAlpha(dripAmount: f32, thickness: f32, viscosity: f32) -> f32 {
    ...
    // Non-[0,1] clamp — keep clamp for specific range
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let time = u.config.x;
    // Audio reactivity — read once, saturate each band
    ...
    // Add specular highlight to slime (wet surface) — branchless via select()
    let specular_active = tint_mask > 0.1;
    let specular_val = 0.3 * tint_mask * (0.5 + 0.5 * sin(uv.y * 100.0 + time * (2.0 + mids * 1.5)));
    finalColor += vec3<f32>(select(0.0, specular_val, specular_active));
