// ═══════════════════════════════════════════════════════════
// Shader: Oil Slick Iridescence
// Category: Image
// Features: audio-reactive, upgraded-rgba, semantic-alpha, depth-aware
// Complexity: Medium
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // Params
    let thicknessBase = mix(0.2, 2.0, u.zoom_params.x) * (1.0 + bass * 0.15);
    ...
    let flowUV = parallaxUV + vec2<f32>(time * flowSpeed * 0.3, time * flowSpeed * 0.2);
    let h      = fbm(flowUV * 4.0, 5) * thicknessBase;

    // Mouse ripple contribution
    let mouse = u.zoom_config.yz;
    let mDist = length(parallaxUV - mouse);
    ...
    let iridCol = vec3<f32>(iR, iG, iB) * iriStr * (1.0 + mid * 0.15);

    // Blend onto source
    var finalRGB = mix(src.rgb, src.rgb * iridCol + iridCol * 0.15, blend);
    finalRGB = clamp(finalRGB, vec3<f32>(0.0), vec3<f32>(1.5));

    // Treble-driven sparkle overlay
    let sparkle = treble * hash3(uv * 800.0 + time).x * 0.2;
