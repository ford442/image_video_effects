...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  let rnd = hash3(cell).xy;
  let sporeDelta = local - (rnd - 0.5) * 0.8;
  let sporeDSq = dot(sporeDelta, sporeDelta);
  let spore = exp(-sporeDSq * (50.0 + treble * 40.0)) * step(0.75, hash3(cell + vec2<f32>(13.3, 7.1)).x);

  let dust = hash3(floor(uv * 300.0 + time * 0.05)).x * exp(-r * 1.8) * nebula;
  ...
  let presence = saturate(armCore * 0.9 + spore * 0.7 + dust * 0.4);
  let alpha = saturate(0.08 + presence * 0.92);
  let depth = saturate(0.92 - armCore * 0.6 - spore * 0.25);
