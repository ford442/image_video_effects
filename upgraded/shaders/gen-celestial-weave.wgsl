...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  let starNoise = hash3(floor((uv + vec2<f32>(time * 0.01, 0.0)) * 240.0)).x;
  ...
  let presence = saturate(fiber * 0.6 + knot * 0.9 + stars);
  let alpha = saturate(0.08 + presence * 0.92);
  let depth = saturate(0.9 - knot * 0.55 - stars * 0.35 + voidDepth * 0.2);
