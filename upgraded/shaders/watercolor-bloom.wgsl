...
// Category: Image
// Features: audio-reactive, temporal, upgraded-rgba, semantic-alpha, depth-aware
// Complexity: Medium
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // Params
    let bloomR    = mix(0.003, 0.025, u.zoom_params.x) * (1.0 + bass * 0.2);
    ...
    let src = textureSampleLevel(readTexture, u_sampler, parallaxUV, 0.0);
    ...
    for (var i = 0; i < 13; i++) {
        bloom += bloomSample(parallaxUV, OFFSETS[i] * ps * (bloomR / ps.x), WEIGHTS[i]);
    }
    ...
    // Treble-driven sparkle on paper highlights
    let sparkle = treble * hash3(uv * 1000.0 + time).x * 0.15;
    col += vec3<f32>(sparkle);

    // Temporal feedback: slowly accumulate / dry
    let prev = textureLoad(dataTextureC, coord, 0);
    let accumulated = mix(vec4<f32>(col, bloom.a), prev, drySpeed * (1.0 - mid * 0.05));
    var finalColor  = accumulated;

    // Semantic alpha — saturate() instead of clamp(..., 0.0, 1.0)
    let alpha = saturate(src.a * (0.85 + luma * 0.15));
    finalColor.a = alpha;
