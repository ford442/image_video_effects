...
files = [
    "anamorphic-caustic-flare.wgsl",
    "astral-kaleidoscope.wgsl",
    "bismuth-crystal-growth.wgsl",
    "crystalline-veins.wgsl",
    "electric-eel-storm.wgsl",
    "fractal-ice-palace.wgsl",
    "gamma-ray-burst.wgsl",
    "holographic-crystal.wgsl",
    "liquid-tensor-vortex.wgsl",
    "photonic-caustics.wgsl"
]
...
for filename in files:
    ...
    # check for exact 'let time' declaration
    has_time = re.search(r'let\s+time\s*=', content) or re.search(r'let\s+t\s*=\s*u\.config\.x', content)
    ...
    # In some files, we might have multiple writeTexture calls. Take the last one.
...
"""
    ...
    textureStore(writeTexture, {coord}, vec4<f32>(__finalRGB, {alpha_var}));"""
            ...
            # Use a slightly robust replacement for dataTextureA as well if it matches
            ...
            # Make sure vec4_var is not "vec4(...)" if we missed it
            if "vec4" in vec4_var:
                # Actually, this might happen if it's vec4<f32>(foo, bar) but didn't match match1 due to some reason
                # But match1 handles vec4 now.
                pass
