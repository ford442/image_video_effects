...
/**
 * Build attract-mode rotation pool: explicit list first, then showcase-tagged,
 * then high-rated generative shaders with full zoom_params mapping.
 */
export function getAttractPool(
  availableModes: ShaderEntry[],
  ratedShaders: RatedShaderRef[] = [],
): ShaderEntry[] {
  const generative = availableModes.filter(
    (s) => s.category === 'generative' && s.id !== 'none',
  );
  const byId = new Map(generative.map((s) => [s.id, s]));
