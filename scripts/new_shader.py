...
PRELUDE_INCLUDE = '#include "_prelude.wgsl"'


def load_template_stub() -> str:
    """
    Return the canonical template body — everything after its prelude include.

    The template gets its bindings and `struct Uniforms` from _prelude.wgsl, so
    the body is just the entry point.
    """
    template = TEMPLATE_PATH.read_text(encoding="utf-8")
    idx = template.find(PRELUDE_INCLUDE)
    if idx == -1:
        raise RuntimeError(
            f"Canonical template missing '{PRELUDE_INCLUDE}' — rebuild {TEMPLATE_PATH}"
        )
    return template[idx + len(PRELUDE_INCLUDE) :].lstrip("\n")
...
//  Bindings and `struct Uniforms` come from _prelude.wgsl — do not paste your
//  own copy. Add `#include "_hash.wgsl"` if you need hash/noise helpers.
// ═══════════════════════════════════════════════════════════════════════════════
