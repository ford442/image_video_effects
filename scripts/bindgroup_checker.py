...
TEMPLATE_FILES = [
    "_hash_library.wgsl",
    # Includable libraries generated by scripts/generate_shader_libs.py. Neither
    # is a shader: _prelude.wgsl is bindings with no entry point, _hash.wgsl is
    # functions with no bindings.
    "_prelude.wgsl",
    "_hash.wgsl",
...
def parse_shader(filepath):
    ...
    # A shader that gets its bindings from _prelude.wgsl has none of its own,
    # so the binding scan below has to run on the expanded source. Files with no
    # directive — all 1417 of them today — come back untouched.
    include_error = None
    if has_wgsl_include(content):
        try:
            content = expand_wgsl_includes(content, entry=filename)
        except WgslIncludeError as exc:
            include_error = str(exc)
    ...
    if include_error is not None:
        ...
        result["errors"].append(f"#include expansion failed: {include_error}")
        return result
