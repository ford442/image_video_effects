#!/usr/bin/env python3
"""Expand Phase B targets with additional candidates."""
...
# Categories ranked by visual impact for prioritization
IMPACT_CATS = ['image', 'generative', 'artistic', 'visual-effects', 'distortion', 'retro-glitch', 'lighting-effects', 'liquid-effects']
...
def main():
    ...
    candidates = [r for r in scan.values()
                  if r["id"] not in PHASE_A
                  and r["id"] not in existing_ids
                  and r["id"] not in EXCLUDE
                  and r["category"] not in ("unknown",)]

    # Split by mouse depth
    no_mouse = [r for r in candidates if r.get("mouse_depth") == "none"]
    basic = [r for r in candidates if r.get("mouse_depth") == "basic"]
    advanced = [r for r in candidates if r.get("mouse_depth") == "advanced"]

    new_targets = []

    # 1. Add all remaining no-mouse shaders
    for r in no_mouse:
        kb = r["size_bytes"] / 1024
        if kb > 15:
            bucket, priority = "huge", 1
        elif kb >= 5:
            bucket, priority = "complex", 2
        else:
            bucket, priority = "mouse_interactive", 4
        new_targets.append(make_target(r, bucket, priority, "Add mouse from scratch."))

    # 2. Add top 50 basic-complex shaders (5-15KB, enhance with clicks/physics)
    basic_complex = [r for r in basic if 5 * 1024 <= r["size_bytes"] <= 15 * 1024 and "-pass" not in r["id"]]
    basic_complex.sort(key=lambda r: (IMPACT_CATS.index(r["category"]) if r["category"] in IMPACT_CATS else 99, -r["size_bytes"]))
    for r in basic_complex[:50]:
        new_targets.append(make_target(r, "complex", 3, "Enhance mouse: add click states, physics, spring following."))

    # 3. Add top 50 basic-simple shaders (<5KB, add click interaction)
    basic_simple = [r for r in basic if r["size_bytes"] < 5 * 1024 and "-pass" not in r["id"]]
    basic_simple.sort(key=lambda r: (IMPACT_CATS.index(r["category"]) if r["category"] in IMPACT_CATS else 99, -r["size_bytes"]))
    for r in basic_simple[:50]:
        new_targets.append(make_target(r, "mouse_interactive", 5, "Enhance mouse: add click-triggered effects."))
    ...
    md = build_md(json_data, all_targets, new_targets)
    with open(PB_MD_PATH, "w") as f:
        f.write(md)

    print("=== PHASE B TARGETS v3 ===")
    print(f"Previous total: {pb['total_targets']}")
...
def build_md(data, targets, new_targets):
    ...
    lines = [
        ...
        "**Version:** 3.0 (expanded with basic-mouse enhancements)",
        ...
        "## Changelog",
        "",
        "- **v1.0:** Initial auto-generated list (114 targets, 39 false positives)",
        "- **v2.0:** Corrected false positives, added mouse-depth classification (113 targets)",
        "- **v3.0:** Expanded with basic-mouse enhancement candidates (+119 targets)",
        "",
        "## Bucket Summary",
        "",
        ...
        f"| **advanced** | {data['mouse_breakdown']['advanced']} | Has clicks/physics — skip or refine |",
        "",
        f"### New in v3.0 ({data['added_in_v3']} added)",
        "",
        "These shaders already read mouse position but lack click states, spring physics, drag forces, or multi-point interaction. Phase B will upgrade them with advanced mouse features.",
        "",
        "---",
        "",
    ...
    for t in targets:
        ...
        marker = " **[NEW]**" if t["id"] in new_ids else ""
        lines.append(f"| {t['priority']} | `{t['id']}`{marker} | {size} | {t['category']} | {md} | {t['notes']} |")
...
def rebuild_registry(pb_data, all_targets):
    ...
    lines.extend([
        ...
        f"| **advanced** | {pb_data['mouse_breakdown']['advanced']} | Has clicks/physics — skip or refine |",
        "",
        "---",
        "",
