...
| historyTex default working size **1024**; upgrade ladder `[[2048,8],[1024,8],[1024,4],[1024,1]]`; sessionStorage `px_history_oom_cap`; 2048 only if `maxBufferSize >= 1GiB` discrete non-Pascal | **#1204** | Parse `vramBudget.ts` + `historyTexProbe.ts` + `HISTORY_DEPTH` against the JSON rungs + `defaultWorkingSize` / `minMaxBufferSizeForFull` |
| Catalog WGSL stays `rgba32float`; WASM rewrites storage decls to the probed colour format (`rgba16float` after a yes probe); BGL bindings 2/7/8 use `RgbaStorageFormat(colorFormat_)`; invalid `CreateComputePipeline` is skip (WaitAny fail = invalid); depth write has `CopySrc` for Feedback Encoder | **#1205** | `wgslFormatRewrite.ts` + `RewriteWgslStorageFormats` + `pipeline.cpp` LoadShader error scope / `waitFailed` + `resources.cpp` Depth Texture Write CopySrc + `frame.cpp` depth CopyTex |
