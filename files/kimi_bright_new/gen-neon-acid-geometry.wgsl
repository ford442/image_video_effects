...
// Glow from SDF
fn sdfGlow(d: f32, width: f32, intensity: f32) -> f32 {
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let pulse = 1.0 + beat * 0.4 * intensity;
    ...
    for (var gx: i32 = -gridCount; gx <= gridCount; gx = gx + 1) {
        for (var gy: i32 = -gridCount; gy <= gridCount; gy = gy + 1) {
            ...
            let glow1 = sdfGlow(abs(shapeDist), 0.012 * intensity * pulse, 2.5);
            let glow2 = sdfGlow(abs(shapeDist), 0.035 * intensity * pulse, 0.8);
            let fill = smoothstep(0.005, -0.005, shapeDist) * 0.6;

            // Additive contribution
            col += shapeCol * glow1 * intensity * 1.5;
            col += shapeCol * glow2 * intensity * 0.5;
            col += shapeCol * fill * intensity * 0.8;
            ...
            if (mouseInfluence > 0.01) {
                ...
                col += acidColor(fract(seed + colorShift + 0.5)) * mouseGlow * intensity * 3.0;
            }
        }
    }
    ...
    let ca = vnoise(uv * 3.0 + time * speed) * 0.03 * intensity;
    col.r += ca * 0.3;
    col.b -= ca * 0.2;
