...
//            depth-aware, domain-warp, hue-shift, semantic-alpha
//  Complexity: High
//  Upgraded: 2026-07-12
// ═══════════════════════════════════════════════════════════════════
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let center = mix(vec2<f32>(0.5), mouse, 0.5 + mouseDown * 0.25);
    ...
    let sampleUV = clamp(scaled + (warp - scaled) * 0.15, vec2<f32>(0.0), vec2<f32>(1.0));
    ...
    let shifted = hsv2rgb(vec3<f32>(fract(hsv.x + hueShiftParam * 0.2 + env * 0.05), hsv.y, hsv.z));
