...
fn map(p: vec3<f32>, time: f32, audio: f32, duneComplexity: f32, windSpeed: f32, geyserHeight: f32, mousePos: vec3<f32>) -> vec2<f32> {
    ...
    // Dune displacement
    let uv_dune = p.xz * 0.5 + vec2<f32>(time * windSpeed, time * windSpeed * 0.5);
    let dune_h = fbm(uv_dune, i32(duneComplexity)) * 2.0;
    d -= dune_h;

    // Audio reactive lift
    d -= audio * 0.5 * fbm(p.xz * 2.0, 3);

    // Mouse gravity crater
    let mouseDist = length(p.xz - mousePos.xz);
    let crater = smoothstep(2.0, 0.0, mouseDist) * 1.5;
    d += crater; // pushes terrain down

    var matId = 0.0; // 0 = sand

    // Geysers (KIFS)
    var q = p;
    q.xz = p.xz - round(p.xz / 4.0) * 4.0; // domain repetition

    var d_geyser = length(q.xz) - 0.2;
    var geyser_y = q.y - (dune_h + 1.0); // start above dunes
    ...
    for(var i=0; i<3; i++) {
        bp.xz = abs(bp.xz) - 0.5;
        let rot = rotate2D(time * 0.5 + f32(i));
        let temp_xz = rot * bp.xz;
        bp.x = temp_xz.x;
        bp.z = temp_xz.y;
        bp.y = abs(bp.y) - 0.5;
    }
    let d_kifs = length(bp) - 0.3 * (1.0 + audio * geyserHeight);
    d_geyser = smax(d_geyser, d_kifs, 0.2);
...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    // Camera
    var ro = vec3<f32>(time * windSpeed, 3.0 + audio, -5.0 + time * windSpeed * 0.5);
    var rd = normalize(vec3<f32>(uv, 1.0));
    let camRot = rotate2D(0.2);
    ...
    let mousePos = ro + vec3<f32>(mouseX * 10.0, 0.0, mouseY * 10.0);
    ...
    for (var i = 0; i < 100; i++) {
        let p = ro + rd * t;
        let d = map(p, time, audio, duneComplexity, windSpeed, geyserHeight, mousePos);
        if (d.x < 0.01) {
            hit = true;
            matId = d.y;
            break;
    ...
    var col = vec3<f32>(0.05, 0.05, 0.1) * max(0.0, 1.0 - uv.y); // Background
    ...
    if (hit) {
        ...
        let rDisp = max(0.0, dot(n, normalize(light1 + vec3<f32>(dispersion * 0.1, 0.0, 0.0))));
        let gDisp = max(0.0, dot(n, light1));
        let bDisp = max(0.0, dot(n, normalize(light1 - vec3<f32>(dispersion * 0.1, 0.0, 0.0))));

        let diff1 = vec3<f32>(rDisp, gDisp, bDisp);
        let diff2 = max(0.0, dot(n, light2)) * vec3<f32>(0.2, 0.3, 0.8);

        if (matId == 0.0) {
            // Sand
            let sandColor = vec3<f32>(0.8, 0.6, 0.3);
            col = sandColor * (diff1 + diff2) * 0.8;
        } else {
            // Geyser
            let geyserColor = vec3<f32>(0.1, 0.8, 0.9) * (1.0 + audio * 2.0);
            col = geyserColor * (diff1 * 1.5 + diff2) + vec3<f32>(1.0, 0.0, 1.0) * audio;
        }
