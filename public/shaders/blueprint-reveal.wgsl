...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let gridSize = 40.0 * (1.0 + mids * 0.12);
    ...
    let finalColor = mix(blueprint, baseColor.rgb, revealMask);
    let alpha = clamp(baseColor.a * 0.45 + (1.0 - revealMask) * 0.32 + edgeVal * 0.18 + bass * 0.05, 0.08, 1.0);
    let depth = clamp(textureSampleLevel(readDepthTexture, non_filtering_sampler, uv, 0.0).r + (1.0 - revealMask) * 0.04, 0.0, 1.0);
