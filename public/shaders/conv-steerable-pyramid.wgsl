...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    // Depth-based sigma (finer detail in foreground)
    let depthSigma = sigma * mix(0.7, 1.3, 1.0 - depth);

    let b0 = steerableFilter(uv, 0.0 + steerOffset + rippleSteer, depthSigma, pixelSize) * responseScale * depthFactor;
    let b45 = steerableFilter(uv, 0.785398 + steerOffset + rippleSteer, depthSigma, pixelSize) * responseScale * depthFactor;
    let b90 = steerableFilter(uv, 1.570796 + steerOffset + rippleSteer, depthSigma, pixelSize) * responseScale * depthFactor;
    let b135 = steerableFilter(uv, 2.356194 + steerOffset + rippleSteer, depthSigma, pixelSize) * responseScale * depthFactor;

    // Chromatic aberration on pyramid bands
    let caStrength = 0.003 * (1.0 + bass);
    let b0_r = steerableFilter(uv + vec2<f32>(caStrength, 0.0), 0.0 + steerOffset + rippleSteer, depthSigma, pixelSize) * responseScale * depthFactor;
    let b90_b = steerableFilter(uv - vec2<f32>(caStrength, 0.0), 1.570796 + steerOffset + rippleSteer, depthSigma, pixelSize) * responseScale * depthFactor;
    ...
    let caColorR = palette(abs(b0_r) * 0.5 + 0.5, vec3<f32>(0.5), vec3<f32>(0.5), vec3<f32>(1.0), vec3<f32>(0.0, 0.33, 0.67)) * abs(b0_r) / (abs(b0_r) + abs(b45) + abs(b90_b) + 0.001);
    let caColorB = palette(abs(b90_b) * 0.5 + 0.5, vec3<f32>(0.5), vec3<f32>(0.5), vec3<f32>(1.0), vec3<f32>(0.67, 0.0, 0.33)) * abs(b90_b) / (abs(b0_r) + abs(b45) + abs(b90_b) + 0.001);
    ...
    color = mix(color, prev.rgb, 0.07 * (1.0 + mids));
