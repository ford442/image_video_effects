...
//  Complexity: Medium
//  Upgraded: 2026-09-09
//  Ideas: log-depth seam walls; even/odd contra-rotate
...
fn tunnelLayer(
  ...
  rotSpeed: f32, distort: f32, layer: i32
) -> vec3<f32> {
  // Idea 2 — even/odd layers contra-rotate
  let spinDir = select(-1.0, 1.0, (layer % 2) == 0);
  let spin = theta * spinDir + t * rotSpeed * (1.0 + bass * 0.5) + z * 0.3;

  let ringMask = sin(r * 30.0 - t * (2.0 + bass * 1.5)) * 0.5 + 0.5;
  let spokeMask = sin(spin * 8.0 + z * 0.5) * 0.5 + 0.5;
  let combined = ringMask * spokeMask;

  // Idea 1 — log-depth seam walls
  let zFrac = fract(z);
  let seam = smoothstep(0.07, 0.0, min(zFrac, 1.0 - zFrac));
  ...
  return col * (combined + arc + seam * 0.85) * (1.0 + warp);
}
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  for (var i = 0; i < layers; i++) {
    ...
    col += tunnelLayer(r, theta, z, t, bass, mids, treble, rotSpeed, distort, i);
  }
  col /= max(f32(layers), 1.0);
  ...
  let alpha = clamp(luma + (1.0 - iris) * 0.05 + srcA * 0.15, 0.0, 1.0);
