...
//  Category: generative
//  Features: audio-reactive, mouse-driven, fireworks, temporal, ring-halo,
...
fn softGlow(uv: vec2<f32>, c: vec2<f32>, r: f32, i: f32) -> f32 {
  let d = length(uv - c);
  return (exp(-d*d/(r*r*0.45)) + 0.3*exp(-d/(r*2.5)))*i;
}
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  // ---- mouse-triggered halo ----
  if (u.zoom_config.w > 0.5) {
    let mAge = fract(time * 0.9) * 3.5;
    if (mAge > 0.8) {
      let mb = mAge - 0.8;
      let mFade = smoothstep(3.5, 0.2, mb);
      let n = i32(50.0);
