...
fn neonGlow(color: vec3<f32>, intensity: f32) -> vec3<f32> {
    ...
    return safeColor + (safeColor * safeColor + chroma) * glowMask * max(intensity, 0.0);
}
...
fn voronoi(p: vec2<f32>) -> vec2<f32> {
    let i = floor(p); let f = fract(p);
    var minDist = 999.0; var secondDist = 999.0;
    for (var y: i32 = -1; y <= 1; y++) {
        for (var x: i32 = -1; x <= 1; x++) {
            let neighbor = vec2<f32>(f32(x), f32(y));
            let point = neighbor + vec2<f32>(h2(i + neighbor), h2(i + neighbor + 7.31));
            let d = dot(neighbor + point - f, neighbor + point - f);
...
// ── Audio envelope smoothing (attack/decay) ──
...
fn metallicColor(pattern: f32, t: f32, bass: f32) -> vec3<f32> {
    ...
    let m = fract(pattern + t * 0.05 + bass * 0.3) * 3.0;
    let s1 = step(1.0, m); let s2 = step(2.0, m);
...
fn gemColor(normal: vec3<f32>, viewDir: vec3<f32>, baseColor: vec3<f32>, t: f32) -> vec3<f32> {
    ...
    let f0 = vec3<f32>(0.17, 0.35, 0.45);
    let fresnel = fresnelSchlick(cosTheta, f0);
    let dispersion = vec3<f32>(1.0 + 0.1 * sin(t * 0.3), 1.0 + 0.05 * sin(t * 0.3 + 1.0), 1.0 + 0.15 * sin(t * 0.3 + 2.0));
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    // Bass pulse scales the field; mids drive rotation speed
    p = rot2(time * 0.05 * (1.0 + mids * 0.5) + projAngle + bass * 0.15) * p;
    ...
    let gemPal = array<vec3<f32>, 5>(
        ...
        vec3<f32>(0.1, 0.8, 0.3), vec3<f32>(0.9, 0.5, 0.1), vec3<f32>(0.7, 0.2, 0.8));
    ...
    // Treble sparkle: high-frequency glints on crystal edges
    let sparkleMask = smoothstep(0.08, 0.0, abs(qc) + abs(qc2) * 0.5);
    col += vec3<f32>(0.6, 0.75, 1.0) * h2(uv * 200.0 + time * 5.0) * sparkleMask * treble * 0.8;
    ...
    col = genChromaticShift(col, uv01, 0.003 * (1.0 + bass) + depthIn * 0.001);
    col = acesToneMap(col * (0.9 + mids * 0.2));

    // Temporal feedback via dataTextureC
    let trail = mix(prev.rgb * 0.96, col, 0.25 + bass * 0.1);
