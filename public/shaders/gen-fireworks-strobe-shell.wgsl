// Strobe Blink Shell — rhythmic multi-flash bursts
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  for (var s: i32 = 0; s < 7; s = s + 1) {
    let si = f32(s); let seed = hash1(si*47.0+7.0);
    ...
    let bDelay = 1.15+seed*0.5; let bAge = max(0.0, age-bDelay);
    ...
    if (bAge > 0.0 && bAge < 6.0) {
      ...
      // Strobing core
      col += flashColor(seed, colorFlash, flash) * softGlow(uv, center, 0.05 + pulse * 0.04, 2.0);

      // Radial sparks — brighter on flash peaks
      let n = i32(22.0 + energy * 28.0 + mids * 10.0);
      for (var j: i32 = 0; j < n; j = j + 1) {
        let jf = f32(j); let js = hash1(si*83.0+jf*3.7);
        ...
        col += flashColor(js+time*0.05, colorFlash, sparkFlash) * softGlow(uv, sp, 0.005+js*0.003, fade*energy);
      }

      // Afterglow phase (post-strobe falling embers)
      if (bAge > 1.5) {
  ...
  if (u.zoom_config.w > 0.5) {
    ...
    if (mAge > 0.5) {
      ...
      col += flashColor(time*0.1, colorFlash, pulse*pulsePow) * softGlow(uv, mUV, 0.06+pulse*0.05, 2.0);
      for (var k = 0; k < 25; k = k + 1) {
        let ang = f32(k)/25.0*TAU;
        let sp = sparkPos(mUV, vec2<f32>(cos(ang),sin(ang))*0.4*pulsePow, mb, 0.85);
        col += flashColor(f32(k)*0.1, colorFlash, pulse) * softGlow(uv, sp, 0.005, pulsePow);
      }
    }
  }
  col = mix(prev*afterglow, col, 0.3);
  col = acesToneMap(col*1.1);
  textureStore(dataTextureB, pixel, vec4<f32>(col*0.55+prev*0.38, 1.0));
  textureStore(dataTextureA, pixel, vec4<f32>(col, 1.0));
  textureStore(writeTexture, pixel, vec4<f32>(col, clamp(length(col)*1.1+0.13, 0.14, 0.97)));
