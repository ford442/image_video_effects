// ═══════════════════════════════════════════════════════════════════
//  Refraction Tunnel — Curvature-Warped Prismatic Tunnel
//  Category: distortion
...
struct Uniforms {
  ...
  zoom_params: vec4<f32>,  // x=TunnelDepth, y=TwistAmount, z=ChromaticDispersion, w=TunnelCurvature
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
  ...
  let mouseOffset = (mouse - 0.5) * curvature * (1.0 + held * 0.35);
  ...
  let newAngle = angle + twistAmount * twistEnvelope + time * rotSpeed * 0.3 * twistEnvelope
               + held * (mouse.x - 0.5) * 1.2 * twistEnvelope;
  ...
  let alpha = clamp(wallProximity * aberration * 3.0 + insideWall * 0.25 + bass * 0.08 + clickRing * 0.3 + held * 0.1, 0.1, 1.0);
