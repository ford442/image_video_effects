...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  // A fast bounded spring converts pointer travel into coherent tear velocity.
  ...
  if (hasSpring && gid.x == 0u && gid.y == 0u) {
    ...
    springVel += ((u.zoom_config.yz - springPos) * 235.0 - springVel * 23.0) * dt;
  ...
  glitchOffset += pointerVelocity / vec2<f32>(aspect, 1.0) * dragCore * select(0.002, 0.012, isMouseDown > 0.5);
