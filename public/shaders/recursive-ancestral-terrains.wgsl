...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // Mouse selects which ancestral "lineage" is active
    // We simulate 3 generations with different characteristics
    ...
    // Audio controls mutation
    let mutation = 0.4 + mids * 0.9 + treble * 0.5;

    // Three different "ancestral" terrains
    ...
    // Blend based on mouse position (lineage selection)
    let terrain = t1 * gen1 + t2 * gen2 + t3 * gen3;

    // Add some interaction between generations
    let blend = gen1 * gen2 + gen2 * gen3 + gen3 * gen1;
    let finalTerrain = terrain + blend * 0.3;

    // Color based on height and lineage
    ...
    let finalCol = mix(col, col * lineageColor * 1.3, 0.4);

    // Alpha based on height variation (ridges more opaque)
    let alpha = clamp(height * 0.85 + abs(finalTerrain - 0.5) * 0.6, 0.2, 1.2);
