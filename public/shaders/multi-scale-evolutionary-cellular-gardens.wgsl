...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let s1 = state.r; // Species 1
    let s2 = state.g; // Species 2
    let resource = state.b;

    // Audio-driven "genetic" pressure
    let mutationRate = 0.3 + mids * 0.9;
    let competition = 0.2 + bass * 0.6;

    // Simple multi-state CA with slow evolution
    let ps = 1.0 / res;
    ...
    // Growth rules that slowly shift with audio
    ...
    // Visualization
    let c1 = vec3<f32>(0.2, 0.6, 0.4) * newS1;
    let c2 = vec3<f32>(0.7, 0.3, 0.5) * newS2;
    let resCol = vec3<f32>(0.3, 0.5, 0.3) * newRes * 0.4;

    let col = c1 + c2 + resCol;

    let totalLife = newS1 * 0.6 + newS2 * 0.6;
    let alpha = clamp(totalLife * 0.85 + newRes * 0.2, 0.15, 1.2);
