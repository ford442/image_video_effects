// ═══════════════════════════════════════════════════════════════════
//  Digital Reveal — Batch 59
//  Spring reveal brush at extraBuffer[133..136] (0,0 writer), exact C mask,
//  capped ripples, held enlarges brush, ACES composite, semantic alpha.
// ═══════════════════════════════════════════════════════════════════
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let revealSize = u.zoom_params.y * select(1.0, 1.25, held);
    ...
    var springPos = mouse;
    let hasSpring = arrayLength(&extraBuffer) > 136u;
    if (hasSpring) {
    ...
    if (global_id.x == 0u && global_id.y == 0u && hasSpring) {
        ...
        if (length(springPos) < 0.0001 && length(springVel) < 0.0001 && mouseValid) {
            springPos = mouse;
        }
        let springGoal = select(springPos, mouse, mouseValid);
        let dt = 0.016;
        let stiffness = 60.0;
  ...
  var rainColor = vec3<f32>(0.0, 1.0, 0.2) * charBright * flicker * (1.0 + plasmaBuffer[band + 1u].x * 0.25);
    rainColor.g += bass * 0.3 * charBright;
