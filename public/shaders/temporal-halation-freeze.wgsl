...
//  Complexity: Medium
//  Upgraded: 2026-09-09
//  Ideas: red dye-layer lag; C freeze-hold
//  A packing: accumulated halo RGB + envelope A (raw bloom; C is previous halo)
// ═══════════════════════════════════════════════════════════════════
...
fn bass_env(prev: f32, bass: f32, attack: f32, release: f32) -> f32 {
    return select(prev * release + bass * (1.0 - release), prev * attack + bass * (1.0 - attack), bass > prev);
}
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    // Idea 1 — red dye-layer lag: C.r holds the halo longer than G/B.
    let dyeDecay = vec3<f32>(mix(decay, min(decay + 0.04, 0.985), 0.85), decay, decay * 0.98);
    var accumulated = prev.rgb * dyeDecay + newHalo * (0.6 + env * 0.3);
    accumulated.r = mix(accumulated.r, prev.r, 0.22);

    // Idea 2 — held pointer freeze-holds the C halo plate.
    let held = step(0.5, u.zoom_config.w);
    accumulated = mix(accumulated, prev.rgb, held * 0.85);
    ...
    let semantic_alpha = clamp(0.58 + energy * 0.55 + held * 0.08 + mids * 0.04, 0.45, 1.0);
    let dither = (ign(vec2<f32>(global_id.xy)) - 0.5) / 255.0;
    let depthOut = clamp(0.18 + energy * 0.45, 0.0, 0.93);

    textureStore(writeTexture, coord, vec4<f32>(col + dither, semantic_alpha));
    textureStore(dataTextureA, coord, vec4<f32>(accumulated, env));
