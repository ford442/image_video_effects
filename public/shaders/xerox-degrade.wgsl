...
// 4×4 Bayer matrix as a closed-form expression (branchless).
...
// Sigmoid contrast curve — perceptually smoother than linear stretch
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let contrast  = u.zoom_params.x * 8.0 + 2.0;
    ...
    // Early exit: negligible effects → fast grayscale pass-through
    if (grain_amt < 0.005 && smear_amt < 0.005 && contrast < 2.5 && threshold < 0.01) {
        let col = textureSampleLevel(readTexture, u_sampler, uv, 0.0).rgb;
        let luma = dot(col, vec3<f32>(0.299, 0.587, 0.114));
        let depth = textureSampleLevel(readDepthTexture, non_filtering_sampler, uv, 0.0).r;
        textureStore(writeDepthTexture, coord, vec4<f32>(depth, 0.0, 0.0, 0.0));
        textureStore(writeTexture, coord, vec4<f32>(vec3<f32>(luma), 1.0));
        return;
    }

    // Packed hash for grain / smear / scatter (single ALU pass)
    let h = hash3_packed(uv * resolution * 0.01 + time * 0.1);
    let smear_n = (h.x - 0.5) * smear_amt * 0.1;

    // Branchless smear with paper-white border fallback
    ...
    // Grayscale → threshold → sigmoid contrast
    ...
    // Toner scatter: branchless via select()
    ...
    // Semantic alpha: paper = translucent, dark toner = opaque
