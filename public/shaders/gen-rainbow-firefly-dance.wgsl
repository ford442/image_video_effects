...
//  Category: generative
//  Features: firefly, rainbow, dance, audio-reactive, mouse-interactive, semantic-alpha
//  Complexity: Medium
//  Created: 2026-05-31
//  Updated: 2026-06-01
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    // Background gradient
    let bgHue = fract(time * 0.02 * speed + colorShift * 0.5);
    ...
    col += hue2rgb(fract(flow + time * 0.03 * speed + colorShift)) * flow * 0.15 * intensity;

    // Firefly count
    let numFireflies = 80;

    // Phosphorescent trail buffer read
    let trailUV = vec2<f32>(pixel) / resolution;
    let trailDecay = 0.94;
    var trailColor = vec3<f32>(0.0);
    if (pixel.x >= 0 && pixel.y >= 0 && pixel.x < i32(resolution.x) && pixel.y < i32(resolution.y)) {
        let prevFrame = textureSampleLevel(readTexture, u_sampler, trailUV, 0.0);
        trailColor = prevFrame.rgb * trailDecay;
    }
    ...
    for (var i: i32 = 0; i < numFireflies; i = i + 1) {
        ...
        // Organic swirling motion - Lissajous-like
        ...
        // Mouse attraction - fireflies swarm toward mouse when down
        ...
        // Scale firefly size
        let fSize = (0.008 + 0.012 * sin(time * 2.0 * speed + fi) * intensity) * scale;

        // Rainbow color - unique per firefly, cycling over time
