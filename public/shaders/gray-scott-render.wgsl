...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  let alpha = clamp(0.25 + V * 0.7, 0.2, 1.0);
  textureStore(writeTexture, pixel, vec4<f32>(clamp(col, vec3<f32>(0.0), vec3<f32>(1.4)), alpha));
  textureStore(writeDepthTexture, pixel, vec4<f32>(clamp(1.0 - V * 0.5, 0.05, 1.0) * max(depth, 0.05), 0.0, 0.0, 0.0));
