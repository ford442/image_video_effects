...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  for (var i = 0u; i < 5u; i = i + 1u) {
    let fi = f32(i);
    let k = knots + fi * 0.5;
    let pa = a + time * spin * (1.0 + bass * 0.5) + fi * 6.28318 / phi;
    let pr = r * (3.0 + fi * phi) - time * (0.3 + mids * 0.4);
    pattern = pattern + smoothstep(0.92, 1.0, abs(sin(pa * k + pr))) * pow(0.7, fi);
  }

  let ring = smoothstep(0.55, 0.75, abs(r - 0.5 + mouse.x * 0.1));
  let node = 0.0;
  for (var i = 0u; i < u32(phiLayers); i = i + 1u) {
    let fi = f32(i);
    let na = a * (knots + fi) + time * spin * phi;
    let nr = 0.25 + fi * 0.12;
    let np = vec2<f32>(cos(na), sin(na)) * nr;
    let nd = length(p - np);
    pattern = pattern + exp(-nd * nd * (200.0 + treble * 150.0)) * (0.6 + fi * 0.15);
  }

  // Chromatic: R golden, G emerald, B sapphire
