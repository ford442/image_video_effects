...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
  ...
  // Use screen-space derivatives to find the gradient of the depth difference.
  // This tells us how sharp the edge is on screen.
  let gradient = length(vec2<f32>(dpdx(dist), dpdy(dist)));
  ...
  final_color.rgb = final_color.rgb * lighting;

  // --- IMPROVEMENT 2: Apply Atmospheric Fog ---
  // Fog is applied after lighting.
  final_color.rgb = calculate_fog(final_depth, final_color.rgb);
