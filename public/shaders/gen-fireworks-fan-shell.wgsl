...
//  Category: generative
//  Features: audio-reactive, mouse-driven, fireworks, temporal, fan-spread,
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  for (var s: i32 = 0; s < 7; s = s + 1) {
    ...
    if (bAge > 0.0 && bAge < 5.5) {
      ...
      // core flash
      col += vec3<f32>(1.0) * exp(-bAge * 10.0) * energy * hexBokeh(uv, center, 0.065, 1.5);
      ...
      for (var j: i32 = 0; j < n; j = j + 1) {
        ...
        let sp = sparkPos(center, vel, bAge, GRAVITY);
        ...
        // faint trailing echo
        let echo = sparkPos(center, vel, max(0.0, bAge - 0.08), GRAVITY);
  ...
  if (u.zoom_config.w > 0.5) {
    ...
    if (mAge > 0.6) {
      ...
      for (var k: i32 = 0; k < 35; k = k + 1) {
        ...
        let sp = sparkPos(mouseUV, vel, mb, GRAVITY);
        let h = fract(f32(k) * 0.1 + hueCycle);
        col += palette(h) * softGlow(uv, sp, 0.006, mFade * power);
      }
    }
  }
