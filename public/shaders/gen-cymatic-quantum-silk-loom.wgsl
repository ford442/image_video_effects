...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  let silkDensity = mix(0.1, 1.0, u.zoom_params.x);
  let cymFreq = mix(0.1, 5.0, u.zoom_params.y);
  let luminescence = mix(0.0, 2.0, u.zoom_params.z);
  let weaveSpeed = mix(0.1, 3.0, u.zoom_params.w);
