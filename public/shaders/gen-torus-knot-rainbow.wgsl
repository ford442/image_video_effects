...
//  Created: 2026-05-23
// ═══════════════════════════════════════════════════════════════════
//  A torus knot (p,q) curve projected onto the 2D plane. The knot
//  rotates, changes winding parameters over time, and is rendered as
//  a thick rainbow-colored ribbon. A secondary unwound ghost trail
//  reinforces the knotting topology.
...
// Torus knot point: p winds around meridian, q around longitude
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
  ...
  let knotP      = floor(mix(2.0, 6.0, u.zoom_params.y));        // meridian winds
  let knotQ      = floor(mix(3.0, 7.0, u.zoom_params.z)) + 1.0;  // longitude winds
  let tubeRadius = mix(0.025, 0.006, u.zoom_params.w);

  var p = (uv - 0.5) * vec2<f32>(aspect, 1.0) * 2.2;

  // Slowly rotating viewpoint
  ...
  let alpha = clamp(length(colorAcc) * 0.7, 0.0, 1.0);
