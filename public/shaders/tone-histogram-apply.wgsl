...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  var color = src.rgb * exposure;
  color = color / (vec3<f32>(1.0) + color);
  color = clamp((color - 0.5) * contrast + 0.5, vec3<f32>(0.0), vec3<f32>(1.0));
  ...
  textureStore(dataTextureA, coord, vec4<f32>(exposure / 3.5, meanLuma, f32(peakBin) / 255.0, saturation));
