...
fn map(p: vec3<f32>, time: f32, mouse_pos: vec3<f32>) -> vec2<f32> {
    ...
    // Add time warp
    thread_p = thread_p + vec3<f32>(sin(time * 0.2), cos(time * 0.15), sin(time * 0.1)) * 0.5;
...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    // Audio reactivity mock (using ripples)
    var audio = 0.0;
    if (u.config.y > 0.0) {
        audio = u.ripples[0].z * 0.1; // mock audio level from first ripple
    }
    ...
    let slow_time = time * 0.1;
    let cam_rot = rot2D(slow_time);
    ...
    if (t < 20.0) {
        ...
        if (mat == 1.0) {
            ...
            // Audio pulse
            let pulse = (sin(time * 5.0 - p.y * 2.0) * 0.5 + 0.5) * acousticGlow * audio;

            col = baseCol + edgeGlow + sss + pulse * vec3<f32>(0.2, 0.6, 1.0);
        } else {
            // Silicate threads
            let iridescenceCol = getPalette(fresnel + time * 0.2 + p.z * 0.1);
