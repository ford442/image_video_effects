...
fn rgbToYuv(rgb: vec3<f32>) -> vec3<f32> {
    ...
    let u_ch = -0.169 * rgb.r - 0.331 * rgb.g + 0.5 * rgb.b + 0.5;
    let v_ch = 0.5 * rgb.r - 0.419 * rgb.g - 0.081 * rgb.b + 0.5;
    return vec3<f32>(y, u_ch, v_ch);
}
...
fn sampleChromaSubsample(uv: vec2<f32>, resolution: vec2<f32>, mode: i32, blockSize: f32) -> vec2<f32> {
    ...
    let blockX2 = floor(uv.x * resolution.x / (blockSize * 2.0)) * blockSize * 2.0 / resolution.x;
    let blockY = floor(uv.y * resolution.y / blockSize) * blockSize / resolution.y;
    
    let blockUV0 = uv;
    let blockUV1 = vec2<f32>(blockX, uv.y);
    let blockUV2 = vec2<f32>(blockX, blockY);
    let blockUV3 = vec2<f32>(blockX2, uv.y);
    
    var blockUV = blockUV0;
    blockUV = select(blockUV, blockUV1, mode == 1);
    blockUV = select(blockUV, blockUV2, mode == 2);
    blockUV = select(blockUV, blockUV3, mode == 3);
...
fn blockGlitch(uv: vec2<f32>, time: f32, intensity: f32) -> vec2<f32> {
    ...
    let activeX = f32(h > 0.85);
    let activeY = f32(h > 0.92);
    let offsetX = (hash1(h) - 0.5) * intensity * 0.3 * activeX;
    let offsetY = (hash1(h + 1.0) - 0.5) * intensity * 0.1 * activeY;
...
fn scanlineGlitch(uv: vec2<f32>, time: f32, intensity: f32) -> f32 {
    ...
    let isActive = f32(h > 0.95);
    return (hash1(h) - 0.5) * intensity * 0.5 * isActive;
}
...
fn rgbShift(uv: vec2<f32>, mouse: vec2<f32>, amount: f32) -> vec3<f32> {
    ...
    let rUV = clamp(uv + shiftDir * shiftAmount, vec2<f32>(0.0), vec2<f32>(1.0));
    let gUV = uv;
    let bUV = clamp(uv - shiftDir * shiftAmount, vec2<f32>(0.0), vec2<f32>(1.0));
...
fn pixelSort(uv: vec2<f32>, time: f32, intensity: f32) -> vec2<f32> {
    ...
    let isActive = f32(h > sortThreshold);
    let sortAmount = (hash1(h) - 0.5) * intensity * 0.2;
    return uv + vec2<f32>(0.0, sortAmount) * isActive;
}
...
fn datamosh(uv: vec2<f32>, mouse: vec2<f32>, time: f32, intensity: f32) -> vec2<f32> {
    ...
    let isActive = f32(h > 0.9);
    let offsetX = (hash1(h) - 0.5) * moshStrength * 0.4;
    return uv + vec2<f32>(offsetX, 0.0) * isActive;
}
...
fn chromaticAberration(uv: vec2<f32>, intensity: f32, channelOffset: f32) -> vec3<f32> {
    ...
    let r = textureSampleLevel(readTexture, u_sampler, clamp(uv + offsetR, vec2<f32>(0.0), vec2<f32>(1.0)), 0.0).r;
    let g = textureSampleLevel(readTexture, u_sampler, clamp(uv + offsetG, vec2<f32>(0.0), vec2<f32>(1.0)), 0.0).g;
    let b = textureSampleLevel(readTexture, u_sampler, clamp(uv + offsetB, vec2<f32>(0.0), vec2<f32>(1.0)), 0.0).b;
    return vec3<f32>(r, g, b);
}
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let audioMids = mids * 0.3;
    let audioTreble = treble * 0.2;
    ...
    let temporalIntensity = u.zoom_params.w * (1.0 + bass * 0.4);
    ...
    var baseColor = textureSampleLevel(readTexture, u_sampler, p, 0.0).rgb;
    var color = baseColor;

    let yuv = rgbToYuv(baseColor);
    let luma = yuv.x;
    let chromaColor = applyChromaSubsampling(p, resolution, chromaMode, blockSize, luma);
    let dctIntensity = dctBlockArtifacts(p, resolution, glitchIntensity * 0.5, blockSize);
    let hasChroma = f32(chromaMode > 0);
    color = mix(color, chromaColor, hasChroma);
    color = mix(color, baseColor, dctIntensity * 0.3 * hasChroma);
    ...
    let scanline = sin(uv.y * scanlineDensity + time * 2.0 * (1.0 + bass * 0.3));
    let scanlinePattern = 0.9 + 0.1 * scanline;
    color = color * scanlinePattern;

    let noise = digitalNoise(uv, time);
    color = mix(color, vec3<f32>(noise), glitchIntensity * 0.1 * (1.0 + bass));

    let bands = 8.0 + (1.0 - glitchIntensity) * 24.0;
    color = floor(color * bands) / bands;

    let flicker = 1.0 + sin(time * 20.0 * (1.0 + bass)) * glitchIntensity * 0.1;
    color = color * flicker;

    let edgeDist = abs(uv.x - 0.5) * 2.0;
    let edgeAberration = edgeDist * glitchIntensity * 0.02;

    let rEdge = textureSampleLevel(readTexture, u_sampler, clamp(p + vec2<f32>(edgeAberration, 0.0), vec2<f32>(0.0), vec2<f32>(1.0)), 0.0).r;
    let bEdge = textureSampleLevel(readTexture, u_sampler, clamp(p - vec2<f32>(edgeAberration, 0.0), vec2<f32>(0.0), vec2<f32>(1.0)), 0.0).b;
    color.r = mix(color.r, rEdge, edgeDist * 0.5);
    color.b = mix(color.b, bEdge, edgeDist * 0.5);

    let barHeight = 0.02;
    let barY = fract(time * 0.3);
    let inBar = step(abs(uv.y - barY), barHeight);
    let barActive = inBar * step(0.3, glitchIntensity);
    ...
    color = mix(color, barColor.rgb, barActive * 0.5);
    ...
    let hasChroma2 = f32(chromaMode > 1);
    color.r = mix(color.r, leftColor.r, glitchIntensity * 0.3 * hasChroma2);
    color.b = mix(color.b, rightColor.b, glitchIntensity * 0.3 * hasChroma2);
    ...
    color = color + vec3<f32>(0.15, 0.12, 0.08) * isBeat * glitchIntensity;
    color = color + vec3<f32>(audioTreble) * glitchIntensity * 0.2;
    ...
    let finalColor = vec4<f32>(finalRGB, clamp(finalAlpha + audioMids, 0.0, 1.0));
