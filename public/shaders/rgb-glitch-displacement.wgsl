...
fn sampleChromaSubsample(uv: vec2<f32>, resolution: vec2<f32>, mode: i32, blockSize: f32) -> vec2<f32> {
    let blockX1 = floor(uv.x * resolution.x / blockSize) * blockSize / resolution.x;
    let blockX2 = floor(uv.x * resolution.x / (blockSize * 2.0)) * blockSize * 2.0 / resolution.x;
    let blockY2 = floor(uv.y * resolution.y / blockSize) * blockSize / resolution.y;
    let isMode1 = f32(mode == 1);
    let isMode2 = f32(mode == 2);
    let isMode3 = f32(mode == 3);
    let blockUV = uv * (1.0 - isMode1 - isMode2 - isMode3)
                + vec2<f32>(blockX1, uv.y) * isMode1
                + vec2<f32>(blockX1, blockY2) * isMode2
                + vec2<f32>(blockX2, uv.y) * isMode3;
...
fn blockGlitch(uv: vec2<f32>, time: f32, intensity: f32) -> vec2<f32> {
    ...
    let doOffsetX = f32(h > 0.85);
    let doOffsetY = f32(h > 0.92);
    let offsetX = (hash1(h) - 0.5) * intensity * 0.3 * doOffsetX;
    let offsetY = (hash1(h + 1.0) - 0.5) * intensity * 0.1 * doOffsetY;
...
fn scanlineGlitch(uv: vec2<f32>, time: f32, intensity: f32) -> f32 {
    ...
    let doScan = f32(h > 0.95);
    return (hash1(h) - 0.5) * intensity * 0.5 * doScan;
}
...
fn pixelSort(uv: vec2<f32>, time: f32, intensity: f32) -> vec2<f32> {
    ...
    let doSort = f32(h > sortThreshold);
    let sortAmount = (hash1(h) - 0.5) * intensity * 0.2;
    return uv + vec2<f32>(0.0, sortAmount) * doSort;
}
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    p = vec2<f32>(p.x + scanOffset, p.y);
    p = pixelSort(p, time, glitchIntensity);
    p = datamosh(p, mouse, time, glitchIntensity);

    let baseColor = textureSampleLevel(readTexture, u_sampler, clamp(p, vec2<f32>(0.0), vec2<f32>(1.0)), 0.0).rgb;
    var color = baseColor;

    let doChroma = f32(chromaMode > 0);
    ...
    let chromaResult = mix(chromaColor, baseColor, dctIntensity * 0.3);
    color = mix(color, chromaResult, doChroma);
    ...
    let scanlineDensity = 20.0 + glitchIntensity * 100.0 + mids * 40.0;
    ...
    let noiseMix = glitchIntensity * 0.1 * (1.0 + audioOverall) * (1.0 + treble * 0.5);
    color = mix(color, vec3<f32>(noise), noiseMix);

    let bands = 8.0 + (1.0 - glitchIntensity) * 24.0 + mids * 12.0;
    color = floor(color * bands) / max(bands, 0.0001);
    ...
    let rSample = textureSampleLevel(readTexture, u_sampler, clamp(p + vec2<f32>(edgeAberration, 0.0), vec2<f32>(0.0), vec2<f32>(1.0)), 0.0).r;
    let bSample = textureSampleLevel(readTexture, u_sampler, clamp(p - vec2<f32>(edgeAberration, 0.0), vec2<f32>(0.0), vec2<f32>(1.0)), 0.0).b;
    color = vec3<f32>(
        mix(color.r, rSample, edgeDist * 0.5),
        color.g,
        mix(color.b, bSample, edgeDist * 0.5)
    );

    let barHeight = 0.02;
    let barY = fract(time * 0.3);
    let inBar = step(abs(uv.y - barY), barHeight);
    let doBar = inBar * f32(glitchIntensity > 0.3);
    ...
    color = mix(color, barColor.rgb, doBar * 0.5);

    let doBleed = f32(chromaMode > 1);
    ...
    color = vec3<f32>(
        mix(color.r, leftColor.r, glitchIntensity * 0.3 * doBleed),
        color.g,
        mix(color.b, rightColor.b, glitchIntensity * 0.3 * doBleed)
    );
    ...
    let baseAlpha = dot(vec3<f32>(alphaR, alphaG, alphaB), luminanceWeights);
    let finalAlpha = clamp(baseAlpha * (1.0 + treble * 0.15), 0.0, 1.0);
