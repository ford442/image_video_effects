...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    let _luma_gm = dot(clamp(finalCol, vec3<f32>(0.0), vec3<f32>(1.0)), vec3<f32>(0.299, 0.587, 0.114));
    let _alpha_gm = clamp(_luma_gm * 0.7 + 0.2, 0.0, 1.0);
    textureStore(writeTexture, id.xy, vec4<f32>(clamp(finalCol, vec3<f32>(0.0), vec3<f32>(2.0)), _alpha_gm));
