// ═══════════════════════════════════════════════════════════════════
//  Spiral Lens — Möbius Kaleidoscope & Domain-Warped Spiral Refraction
//  Category: distortion
...
struct Uniforms {
  ...
  zoom_params: vec4<f32>,  // x=SpiralTightness, y=LensStrength, z=Chromatic, w=RotationSpeed
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
  ...
  for (var i: u32 = 0u; i < rippleCount; i = i + 1u) {
    ...
    if (rp.z > 0.0 && age >= 0.0 && age < 1.6) {
      irisWave = max(irisWave, exp(-abs(distance(uv01, rp.xy) - age * 0.46) * 72.0) * (1.0 - age / 1.6));
    }
  }
  ...
  let liquidRainbow = 0.5 + 0.5 * cos(TAU * (vec3<f32>(filmPhase * 0.08) + vec3<f32>(0.0, 0.333, 0.667)));
  let mobiusCaustic = pow(max(0.0, sin(length(mz) * 18.0 - angle * 4.0 - time * 4.2)), 6.0) * lensMask;
  ...
  alpha = clamp(alpha + held * 0.1, 0.06, 1.0);

  let overlay = (rainbow + causticLight + bloom + armColor + liquidRainbow * (mobiusCaustic * 0.3 + irisWave * 0.38)) * alpha;
  var color = col + overlay;

  // Exact dataTextureC persistence
  let prevC = textureLoad(dataTextureC, pixel, 0).rgb;
  color = mix(color, prevC, 0.08);

  let finalRGB = acesToneMap(color);
