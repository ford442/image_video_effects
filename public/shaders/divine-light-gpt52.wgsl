...
//  Chunks From: domain-warped FBM, Voronoi F2-F1, curl noise
//  Created: 2026-05-10
// ═══════════════════════════════════════════════════════════════════
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
  ...
  for (var i = 0; i < steps; i = i + 1) {
    ...
    accum += sample * weight * intensity * audioBoost * contrib * phase;
    ...
    current += delta + flow * delta * 0.3;
    weight *= decay;
  }
  accum *= 1.0 / max(f32(steps), 1.0) * 0.9;
