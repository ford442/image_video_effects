...
// 18×18 = 324 f32 values (1.3 KB) - fits in local memory with great performance
// ═══════════════════════════════════════════════════════════════════════════════
...
// Sample from shared memory (fast!)
...
// Calculate Laplacian (∇²h) using 5-point stencil from shared memory
...
// Calculate biharmonic operator (∇⁴h) from shared memory
// Much faster: uses cached Laplacian values
fn biharmonicShared(lid: vec3<u32>) -> f32 {
  // First compute Laplacian at center and neighbors
  let centerLap = laplacianShared(lid);
  
  // For neighbors, we need their Laplacians
  // We can compute these directly from shared memory without additional barriers
  var leftLap = 0.0;
  var rightLap = 0.0;
  var bottomLap = 0.0;
  var topLap = 0.0;
  
  // Left neighbor's Laplacian
  {
...
// Calculate surface normal from height gradient using shared memory
...
// ═══════════════════════════════════════════════════════════════════════════════
// ORIGINAL FUNCTIONS (fallback for edge cases)
// ═══════════════════════════════════════════════════════════════════════════════
...
fn main(
  ...
  // ═══════════════════════════════════════════════════════════════════════════════
  // CAPILLARY WAVE PHYSICS USING SHARED MEMORY - REDUCED DIVERGENCE
  // ═══════════════════════════════════════════════════════════════════════════════
  
  // Pre-compute masks for branchless execution
  let activeMask = f32(backgroundFactor > 0.01);  // 1.0 if active, 0.0 if not
  
  // Calculate spatial derivatives using shared memory (always, but masked later)
  let lapH = laplacianShared(lid);
  let biLapH = biharmonicShared(lid);
  
  // Capillary wave acceleration
  ...
  // Semi-implicit Euler integration (branchless using mix)
  ...
  // Add meniscus effect (branchless)
  newHeight += meniscusEffect(uv, depth, surfaceTension) * 0.1 * activeMask;
  
  // Soft clamp (only applied when active)
  ...
  // ═══════════════════════════════════════════════════════════════════════════════
  // RIPPLE SOURCES (Mouse-driven + Ambient) - REDUCED DIVERGENCE
  // ═══════════════════════════════════════════════════════════════════════════════
  var sourceHeight = 0.0;
  var sourceVelocity = 0.0;
  
  // Mouse-driven Ripples - FLATTENED LOOP (less divergence)
  // Dynamic ripple count from uniform (already the case)
  let rippleCount = u32(u.config.y);
  
  // Unroll-friendly loop with masks instead of nested ifs
  for (var i: u32 = 0u; i < rippleCount; i = i + 1u) {
    ...
    // Compute mask: valid distance (avoid singularity)
    ...
    // Accumulate with mask (branchless)
  ...
  // Ambient Capillary Waves - BRANCHLESS
  // Always compute, but mask by backgroundFactor
  ...
  var ambientHeight = microRipple * 0.003 * microEnvelope * surfaceTension * ambientMask;
  ...
  // Calculate liquid properties from height field
  let liquidHeight = abs(newHeight) * 2.0;
  let liquidThickness = liquidHeight * (1.0 + turbidity);
  
  // Calculate normal for lighting using shared memory
  ...
  // Write persistent height field (velocity Verlet integration)
  textureStore(dataTextureA, gid.xy, vec4<f32>(h_curr, newHeight, newVelocity, age + dt));
  
  // Write depth (unchanged)
