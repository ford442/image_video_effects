...
fn proceduralHeight(
    ...
    for (var i: u32 = 0u; i < rippleCount; i = i + 1u) {
        ...
        let phase = dist * 20.0 * gravityScale - timeSinceClick * 5.0 * gravityScale;
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let hCenter = proceduralHeight(uv, currentTime, surfaceTension, gravityScale, damping, backgroundFactor, bass, mids);
    let hLeft = proceduralHeight(uv - vec2<f32>(pixelSize.x, 0.0), currentTime, surfaceTension, gravityScale, damping, backgroundFactor, bass, mids);
    let hRight = proceduralHeight(uv + vec2<f32>(pixelSize.x, 0.0), currentTime, surfaceTension, gravityScale, damping, backgroundFactor, bass, mids);
    let hBottom = proceduralHeight(uv - vec2<f32>(0.0, pixelSize.y), currentTime, surfaceTension, gravityScale, damping, backgroundFactor, bass, mids);
    let hTop = proceduralHeight(uv + vec2<f32>(0.0, pixelSize.y), currentTime, surfaceTension, gravityScale, damping, backgroundFactor, bass, mids);
    ...
    let specular = pow(specAngle, 128.0) * 0.8 * (1.0 - turbidity * 0.5) * (1.0 + treble * 0.5);
