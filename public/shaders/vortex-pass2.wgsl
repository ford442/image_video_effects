...
//  Category: distortion
//  Features: multi-pass-2, motion-blur, iridescent vorticity, palette grading, upgraded-rgba
//  Complexity: Medium
//  Created: Phase B / Visualist
//  Upgraded: 2026-05-23
// ═══════════════════════════════════════════════════════════════════
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let iridescence    = clamp(u.zoom_params.z, 0.0, 1.0) * (1.0 + treble * 0.2);
    ...
    let displacedUV = clamp(uv + velocity * displacementScale, vec2<f32>(0.0), vec2<f32>(1.0));
    ...
    let baseUV = clamp(displacedUV + swirlRot, vec2<f32>(0.0), vec2<f32>(1.0));
    ...
    for (var i = -2; i <= 2; i++) {
        let t = f32(i) * 0.5;
        let w = exp(-t * t);
        let sampleUV = clamp(baseUV + blurStep * t, vec2<f32>(0.0), vec2<f32>(1.0));
    ...
    let finalAlpha = clamp(0.55 + bloom * 0.4 + velMag * 0.25 - scatter + mids * 0.1, 0.2, 1.0);
    ...
    let depthSample = textureSampleLevel(readDepthTexture, non_filtering_sampler, baseUV, 0.0).r;
    let depthMod = 1.0 + velMag * 0.1 * vortexStrength;
    textureStore(writeDepthTexture, global_id.xy, vec4<f32>(depthSample * depthMod, 0.0, 0.0, 0.0));
}
