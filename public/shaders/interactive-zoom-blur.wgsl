...
struct Uniforms {
  ...
  zoom_params: vec4<f32>,  // x=BlurStrength, y=Samples, z=CenterBias, w=Aberration
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
  ...
  // Parameters
  let blurStrength = u.zoom_params.x * 0.1 * max(1.0 + bass * 0.2, 0.001);
  let samples = i32(mix(4.0, 32.0, clamp(u.zoom_params.y, 0.0, 1.0)));
  let centerBias = clamp(u.zoom_params.z, 0.0, 1.0); // 0 = linear, 1 = concentrated at center
  ...
  for (var i = 0; i < samples; i++) {
      ...
      var weight = 1.0;
      if (centerBias > 0.0) {
          weight = max(mix(1.0, 1.0 - t, centerBias), 0.001);
      }
