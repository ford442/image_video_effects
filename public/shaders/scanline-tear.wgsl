// ═══════════════════════════════════════════════════════════════════
//  Scanline Tear — Batch 62
//  NTSC interlace + H/V sync: spring tracking focus, held boost,
//  capped click sync-loss ripples, exact C field memory, ACES output.
// ═══════════════════════════════════════════════════════════════════
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
  ...
  for (var i = 0u; i < rippleCount; i = i + 1u) {
    ...
    let age = time - rp.z;
    if (age >= 0.0 && age < 1.0) {
      syncPunch = max(syncPunch, smoothstep(0.12, 0.0, length((uv - rp.xy) * vec2<f32>(aspect, 1.0))) * (1.0 - age));
    }
  }

  let interlaceStrength = clamp(u.zoom_params.x + mids * 0.1, 0.0, 1.0) * select(1.0, 1.25, held);
  let hsyncError = clamp(u.zoom_params.y + bass * 0.4 + syncPunch * 0.35, 0.0, 1.0);
  let vsyncError = clamp(u.zoom_params.z + bass * 0.2 + syncPunch * 0.2, 0.0, 1.0);
  ...
  var sampleUV = uv;
  let mousePull = (smoothMouse - uv) * exp(-length((uv - smoothMouse) * vec2<f32>(aspect, 1.0)) * 6.0) * 0.02 * select(0.0, 1.0, held);
  ...
  var prevFrameSample = textureLoad(dataTextureC, coord, 0);
  ...
  if (colorBurstError > 0.0) {
    ...
    let fftPhase = plasmaBuffer[bandBin].x * 0.1;
    outputColor = vec4<f32>(
      colorBurstShift(outputColor.rgb, phaseError + lineHueShift + fftPhase),
      outputColor.a
    );
  ...
  outputColor = vec4<f32>(acesToneMap(clamp(outputColor.rgb, vec3<f32>(0.0), vec3<f32>(1.0)) * (0.96 + mids * 0.04)), outputColor.a);
  let alpha = clamp(outputColor.a + syncPunch * 0.15 + combAmount * 0.1, 0.0, 1.0);
  outputColor.a = alpha;
