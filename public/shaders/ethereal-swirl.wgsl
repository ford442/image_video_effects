...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    textureStore(dataTextureA, vec2<i32>(gid.xy), vec4<f32>(__finalRGB, vec4<f32>(finalColor, 1.0).a));
    ...
    let __finalRGB = vec4<f32>(finalColor, 1.0).rgb + spectral * (abs(clockRings) * 0.1 + clickFront * 0.25);
    textureStore(writeTexture, vec2<i32>(gid.xy), vec4<f32>(__finalRGB, vec4<f32>(finalColor, 1.0).a));
